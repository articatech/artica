...
$('#flexRT$t').flexigrid({
	...
	colModel : [
		...
		{display: '$destination', name : 'description', width :225, sortable : true, align: 'left'},
...
$data = array();
	...
	$data['rows'][] = array(
		...
		"<span style='font-size:14px'>$relay</span>",
		$delete )
		);
	}
	...
	for($i=0;$i<$hash["count"];$i++){
		...
		$data['rows'][] = array(
			'id' => "dom-$domain",
			'cell' => array("
			<a href=\"javascript:blur();\" OnClick=\"javascript:$js\" style='font-size:16px;font-weight:bold;text-decoration:underline'>$domain</span>",
