...
$html="
	...
	function GlobalSettingsPagePopup(){
		YahooWin(930,'$page?js-index=yes','$title');
	}		
...
function SMTP_PERFORMANCES(){
	...
	<tr>
	<td colspan=2 align='right'><input type='button' value='{apply}&nbsp;&raquo;' OnClick=\"javascript:ArticaMailAddonsLevel_save();\"></td>
	</tr>
...
<form name='FFM119'>
	...
	<tr>
		...
		<input type='button' OnClick=\"javascript:HTTPS_PROCESSES();\" value='{apply}&nbsp;&raquo;'></td>
	</tr>
</table>
</form>";	
...
<div id='notifcclist' style='width:100%;height:110px;overflow:auto'>$notifcclist</div>
</div>
<script>
...
<tr>
	<td valign='top' colspan=2 align='right'>".button('{apply}',"javascript:SavePostFixNotifications()")."</td>
</tr>
</table>
</div>
<script>
...
if($ini->_params["PERFORMANCES"]["ArticaMaxLogsSize"]==null){$ini->_params["PERFORMANCES"]["ArticaMaxLogsSize"]=300;}
//
$form="<table style='width:90%;margin:5px;padding:5px'>
		...
		<tr>
			<td align='right' colspan=3>
				<input type='button' value='{apply}&nbsp;&raquo;' OnClick=\"javascript:ArticaWebRootURI();\">
			</td>
		</tr>
		</table>";	
...
<input type='hidden' name='ARTICA_FILTER_MAXSUBQUEUE' id='ARTICA_FILTER_MAXSUBQUEUE' value='$ArticaMaxSubQueueNumber'>
	<center>
		...
		<tr><td align='right'><input type='submit' value='{apply}&nbsp;&raquo;'></td></tr>
...
function ChangeGlobalAdminPassword(){
	...
	var password=encodeURIComponent(document.getElementById('change_password').value);
...
function LDAP_CONFIG(){
	...
	while (list ($num, $ligne) = each ($array) ){
		if($num=="ldap-proxy"){
			$html[]= "<li><a href=\"openldap.proxy.php\"><span style='font-size:14px'>$ligne</span></a></li>\n";
			continue;
			
		}
		
		$html[]= "<li><a href=\"$page?main-ldap=$num\"><span style='font-size:14px'>$ligne</span></a></li>\n";
...
function LDAP_CONFIG_BDBD(){
	...
	<div style='font-size:18px'>{ldap_configure_bdbd}</div>
	<table style='width:99%' class=form>
<tr>
			<td class=legend style='font-size:16px'>{set_cachesize}:</td>
			<td style='font-size:16px'>". Field_text('set_cachesize',$LdapDBSetCachesizeMo,'width:60px;font-size:16px')."M&nbsp;</td>
		</tr>
		<tr>
			<td class=legend style='font-size:16px'>{ldap_cache_size}:</td>
			<td style='font-size:16px'>". Field_text('cachesize',$LdapDBSCachesize,'width:90px;font-size:16px')."&nbsp;{entries}</td>
		</tr>		
		
		<tr>
			<td colspan=2 align='right'><hr>
			". button("{apply}","ParseFormLDAPDB()","18px")."
			</td>
		</tr>	
	</table>
	</div>	
...
function LDAP_CONFIG_NET_LIST(){
	...
	if(is_array($nets)){
		...
		<tr>
			...
			<td width=1%>". button("{add}","ldapAddNet()","14px")."</td>
		</tr>
		</table>	
		";
	}
	...
	if(is_array($SavedNets)){
		...
		while (list($num,$ip)=each($SavedNets)){
			...
			<td><strong><code style='font-size:16px'>$ip</td>
...
$db="<table style='width:99%' class=form>
		<tr>
			<td class=legend style='font-size:16px'>{database_size}:</td>
			<td><strong style='font-size:16px' nowrap>$LdapdbSize</td>
		</tr>";	
	...
	if(is_array($dbstat)){
		while (list ($num, $ligne) = each ($dbstat) ){
			...
			if(preg_match("#^([0-9]+)\s+(.+)#",$ligne,$re)){
				...
				$db=$db."<tr>
					<td class=legend style='font-size:16px' nowrap>{$re[2]}:</td>
					<td><strong style='font-size:16px'>{$re[1]}</strong></td>
					</tr>";
			}
		}
	}
...
function LDAP_CONFIG_SETTINGS(){
	...
	<table style='width:99%' class=form>
		...
		<tr>
			<td colspan=2 align='right'><hr><input type='button' OnClick=\"javascript:ParseFormLDAP();\" value='{apply}&nbsp;&raquo'></td>
		</tr>	
	</table>
	";
