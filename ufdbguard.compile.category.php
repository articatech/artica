...
function title(){
	...
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("{compile2}::{category}::{$_GET["category"]}");
	return $title;
}
...
function Launch(){
	...
	$sock->getFrameWork("ufdbguard.php?compile-category=yes&category={$_GET["category"]}");
}
...
function popup(){
	...
	$title="{please_wait}, {compile2}::{category}::{$_GET["category"]}";
