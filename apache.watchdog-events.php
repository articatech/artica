...
function ShowID_js(){
	...
	$sql="SELECT subject FROM apache_admin_mysql WHERE ID=$id";
	$q=new mysql();
...
$tpl=new templates();
$sql="SELECT content FROM apache_admin_mysql WHERE ID={$_GET["ShowID"]}";
$q=new mysql();
$ligne=@mysql_fetch_array($q->QUERY_SQL($sql,"artica_events"));
...
function empty_table(){
	$q=new mysql();
	$q->QUERY_SQL("TRUNCATE TABLE apache_admin_mysql","artica_events");
}
