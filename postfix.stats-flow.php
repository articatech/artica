...
function build_query_js(){
	...
	$t=$_GET["t"];
	$user=$_GET["user"];
	$md5=md5("SMTP_FLOW:$from$to$interval$user{$_GET["searchsites"]}{$_GET["searchuser"]}{$_SESSION["uid"]}");
	...
	$nextFunction="LoadAjax('SMTP_FLOW_STATS_MAIN_GRAPH','$page?main=yes&zmd5=$md5&t=$t');";
	...
	if(intval($ligne["ID"])==0){
		...
		$title="{flow}: $timetext1 - {to} $timetext2 email {like} {$_GET["searchuser"]}";
		$sql="INSERT IGNORE INTO `reports_cache` (`zmd5`,`title`,`report_type`,`zDate`,`params`,`ou`,`uid`) VALUES 
		('$md5','$title','SMTP_FLOW_MD',NOW(),'$serialize','$ou','$uid')";
		$q->QUERY_SQL($sql);
...
function page(){
	...
	if($_GET["zmd5"]==null){
		$q=new mysql_squid_builder();
		$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT title,zmd5 FROM reports_cache WHERE report_type='SMTP_FLOW_MD' ORDER BY zDate DESC LIMIT 0,1"));
	}else{
		$q=new mysql_squid_builder();
	...
	if($ligne["zmd5"]<>null){
		$nextFunction="LoadAjax('SMTP_FLOW_STATS_MAIN_GRAPH','$page?main=yes&zmd5={$ligne["zmd5"]}&t=$t');";
		$content=null;
	...
	$html="
	<div id='SMTP_FLOW_STATS_MAIN_GRAPH'>$content</div>
	<script>
...
function graph1(){
	...
	$highcharts->xAxis=$x;
	$highcharts->Title="{messages_number}";
...
function graph2(){
	...
	$results=$q->QUERY_SQL("SELECT SUM(size) as size,zdate FROM \"$table\" GROUP BY zdate ORDER BY zdate");
	while($ligne=@pg_fetch_assoc($results)){
	...
	$highcharts->xAxis=$x;
	$highcharts->Title="{messages_size}";
	$highcharts->TitleFontSize="22px";
	$highcharts->AxisFontsize="12px";
...
function table2(){
	...
	$html[]=$tpl->_ENGINE_parse_body("<tr><th style='font-size:18px;padding:8px'>{top} {sender}/{size}</td>
			<th style='font-size:18px'>{size}</td></tr>");
	
	$results=$q->QUERY_SQL("SELECT SUM(size) as size,mailfrom FROM \"$table\" GROUP BY mailfrom ORDER BY size DESC LIMIT 15");
	while($ligne=@pg_fetch_assoc($results)){
		$site=$ligne["mailfrom"];
...
function graph3(){
	...
	$q=new postgres_sql();
	$results=$q->QUERY_SQL("SELECT SUM(size) as size,mailto FROM \"$table\" GROUP BY mailto ORDER BY size DESC LIMIT 10");
	...
	$highcharts->Title=$tpl->_ENGINE_parse_body("TOP {recipients}/{size}");
	echo $highcharts->BuildChart();
	echo "\n";
	echo "if(document.getElementById('websites-button-area')){document.getElementById('websites-button-area').innerHTML='';}\n";
