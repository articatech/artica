...
if(isset($_POST["save-exportpass"])){save_exportpass();exit;}
...
function title(){
	$tpl=new templates();
	return $tpl->javascript_parse_text("{generate_pfx} {$_GET["CommonName"]}");
}


function suffix(){
	return "&CommonName=".urlencode($_GET["CommonName"]);
}
...
function save_exportpass(){
	$sock=new sockets();
	if($_POST["save-exportpass"]==null){return;}
	$sock->SET_INFO("PfxPassword", $_POST["save-exportpass"]);
...
function Launch(){
	$sock=new sockets();
	$cmd="openssl.php?generate-pfx=".urlencode($_GET["CommonName"]);
...
function popup(){
	...
	$title="{PLEASE_WAIT_GENERATING_CERTIFICATE}";
