...
if(isset($argv[1])){$_GET[$argv[1]]="yes";
echo "_GET {$argv[1]}\n";
...
if(isset($_GET["system-shutdown"])){shell_exec("shutdown -h now");exit;}
...
if(isset($_GET["zarafa-stop-web"])){zarafa_stop_web();exit;}
...
function import_computer_from_list(){
	...
	$GLOBALS["CACHEFILE"]="/usr/share/artica-postfix/ressources/logs/web/ocs.import.progress";
	$GLOBALS["LOGSFILES"]="/usr/share/artica-postfix/ressources/logs/web/ocs.import.progress.txt";
...
function StartAllServices(){
	...
	$unix=new unix();
	$nohup=$unix->find_program("nohup");
	shell_exec("$nohup /etc/init.d/artica-postfix start >/dev/null 2>&1 &");
	while (list ($num, $cmd) = each ($d)){
		shell_exec("$nohup /etc/init.d/artica-postfix start $cmd >/dev/null 2>&1 &");	
	}
...
function SQUID_STATUS(){
	$cmd=LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.status.php --squid --nowachdog 2>&1";
	exec("$cmd",$results);
	echo "<articadatascgi>". implode("\n",$results)."</articadatascgi>";
}
...
function SQUID_INI_STATUS(){
	$unix=new unix();
	$cmd=LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.status.php --all-squid --nowachdog";
	exec($cmd,$results);
	writelogs_framework($cmd." ->".count($results)." lines",__FUNCTION__,__FILE__,__LINE__);
	@file_put_contents("/usr/share/artica-postfix/ressources/databases/ALL_SQUID_STATUS", @implode("\n", $results));
	echo "<articadatascgi>". base64_encode(implode("\n",$results))."</articadatascgi>";
}
...
function RoundCube_sieverules(){
	NOHUP_EXEC( LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.roundcube.php --sieverules");
}
function RoundCube_contextmenu(){
	NOHUP_EXEC( LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.roundcube.php --contextmenu");
}


function RoundCube_sync(){
	$ou=base64_decode($_GET["ou"]);
	NOHUP_EXEC( LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.roundcube.php");
	NOHUP_EXEC( LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.groupwares.db.php \"$ou\"");
}



function RoundCube_calendar(){
	NOHUP_EXEC( LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.roundcube.php --calendar");
}
function RoundCube_globaladdressbook(){
	NOHUP_EXEC( LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.roundcube.php --addressbook");
}

function RoundCube_hack(){
	NOHUP_EXEC( LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.roundcube.php --hacks");
}

function opengoo_user(){
	NOHUP_EXEC( LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.opengoo.php --user={$_GET["opengoouid"]}");
}
function groupoffice_user(){
	NOHUP_EXEC( LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.GroupOffice.php --user={$_GET["GroupOfficeUid"]}");
}
function ntpd_restart(){
	NOHUP_EXEC("/etc/init.d/artica-postfix restart ntpd");
}
...
function ReloadArticaFilter(){
	NOHUP_EXEC( LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.postfix.maincf.php --artica-filter --reload");
}

function postfix_perso_settings(){
	NOHUP_EXEC( LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.postfix.maincf.php --perso-settings --reload");
}
...
function postfix_smtpd_restrictions(){
	...
	NOHUP_EXEC( LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.postfix.maincf.php --smtpd-restrictions --reload");
}
...
function mempy(){
	...
	@chmod("/usr/share/artica-postfix/bin/ps_mem.py",0755);
	shell_exec("/usr/share/artica-postfix/bin/ps_mem.py >$tmpdir/mempy.txt 2>&1");
	echo "<articadatascgi>". @file_get_contents("$tmpdir/mempy.txt")."</articadatascgi>";
}
...
function AptGetUpgrade(){
	NOHUP_EXEC( LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.apt-get.php --upgrade");
}
...
function ForceRefreshRight(){
	NOHUP_EXEC( LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.admin.smtp.flow.status.php --services");	
}
...
function samba_restart(){
	NOHUP_EXEC("/etc/init.d/artica-postfix restart samba");	
}
...
function RestartDaemon(){
	...
	$unix->THREAD_COMMAND_SET("/etc/init.d/artica-framework restart");
	
}
function RestartFetchmail(){
	$unix=new unix();
	$nohup=$unix->find_program("nohup");
	$cmd=trim(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.shm.php --SessionMem --force");
	writelogs_framework("$cmd",__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);
...
function SQUIDGUARD_WEB_RELOAD(){
	...
	$cmd=trim("$nohup $php5 /usr/share/artica-postfix/exec.ufdbclient.reload.php >/dev/null 2>&1 &");
	writelogs_framework("$cmd",__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);
...
function postfix_multi_configure_hostname(){
	...
	$php5=$unix->LOCATE_PHP5_BIN();
	$nohup=$unix->find_program("nohup");
	$cmd=trim("$nohup $php5 /usr/share/artica-postfix/exec.postfix-multi.php --instance-reconfigure {$_GET["postfix-multi-configure-hostname"]} >/dev/null 2>&1 &");
...
function RestartApacheNow(){
	...
	$datas=NOHUP_EXEC("/etc/init.d/artica-postfix restart apache &");
	error_log("[{$_SESSION["uid"]}]::restarting apache done $datas");
}
...
function testnotif(){
	$unix=new unix();
	$tmpstr=$unix->FILE_TEMP();
	$cmd=LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/cron.notifs.php --sendmail >$tmpstr 2>&1";
	writelogs_framework($cmd,__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);
...
function FreeCache(){
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.swap-monitor.php --free");	
}
...
function kavmilter_configure(){
	if(is_file("/opt/kav/5.6/kavmilter/bin/kavmilter")){
		NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.kavmilter.php");
	}
		
if(is_file("/opt/kaspersky/kav4lms/bin/kav4lms-cmd")){
		NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.kav4mls.php");
	}	
...
function  philesight_perform(){
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.philesight.php --rebuild");
}

function PostfixHeaderCheck(){
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.postfix.maincf.php --headers-check");
}
...
function squid_rebuild_reconfigure(){
	$unix=new unix();
	$nohup=$unix->find_program("nohup");
	$php5=$unix->LOCATE_PHP5_BIN();
	...
	$cachefile="/usr/share/artica-postfix/ressources/logs/web/squid_reconfigure.txt";
	@unlink($cachefile);
	@file_put_contents($cachefile, "Please Wait....\n");
	@chmod($cachefile, 0755);
	$restart=null;
	if($_GET["restart"]=="yes"){$restart=" --restart"; }


	$cmd="$nohup $php5 /usr/share/artica-postfix/exec.squid.reconfigure.php >$cachefile 2>&1 &";
	writelogs_framework($cmd,__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);
}
...
function squid_rebuild(){
	...
	$cmd="$nohup $php5 /usr/share/artica-postfix/exec.squid.php --build --force$restart >>$cachefile 2>&1 &";
	writelogs_framework($cmd,__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);
}
...
function TCP_VIRTUALS(){
	...
	if(isset($_GET["stay"])){
		if($_GET["stay"]<>"no"){
			...
			shell_exec("$php5 /usr/share/artica-postfix/exec.virtuals-ip.php >/usr/share/artica-postfix/ressources/logs/web/exec.virtuals-ip.php.html 2>&1");
	...
	$php5=$unix->LOCATE_PHP5_BIN();
	@unlink("/etc/artica-postfix/MEM_INTERFACES");
	$cmd="$nohup $php5 /usr/share/artica-postfix/exec.virtuals-ip.php >/usr/share/artica-postfix/ressources/logs/web/exec.virtuals-ip.php.html 2>&1 &";
	@chmod("/usr/share/artica-postfix/ressources/logs/web/exec.virtuals-ip.php.html",0777);
	writelogs_framework($cmd,__FUNCTION__,__FILE__,__LINE__);
	shell_exec(trim($cmd));
	
	shell_exec("$nohup /etc/init.d/dnsmasq restart >/dev/null 2>&1 &");
	shell_exec("$nohup /etc/init.d/ssh restart >/dev/null 2>&1 &");
...
function MalwarePatrol(){
	...
	NOHUP_EXEC("/usr/share/artica-postfix/bin/artica-update --MalwarePatrol");
	}
...
function MalwarePatrol_list(){
	...
	if($pattern==null){
		...
		$cmd="$grep -i -E '$pattern' /etc/squid3/malwares.acl 2>&1";
	}
	writelogs_framework($cmd,__FUNCTION__,__FILE__,__LINE__);
	exec($cmd,$results);
	echo "<articadatascgi>". base64_encode(serialize($results))."</articadatascgi>";
...
function postfix_body_checks(){
	...
	if($hostname=="master"){
		NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.postfix.maincf.php --body-checks");
		return;
	}
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.postfix-multi.php --instance-reconfigure \"$hostname\"");
}
...
function postfix_postqueue_reprocess_msgid(){
	$unix=new unix();
	$postsuper=$unix->find_program("postsuper");
	$logpath="/usr/share/artica-postfix/ressources/logs/web/postcat-{$_GET["postsuper-r-master"]}.log";
...
function kas_reconfigure(){
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.kasfilter.php");
}
...
function POWERDNS_RESTART(){
	...
	$php5=$unix->LOCATE_PHP5_BIN();
	$nohup=$unix->find_program("nohup");
	shell_exec("$php5 /usr/share/artica-postfix/exec.initslapd.php --pdns >/dev/null 2>&1");
...
function Kav4ProxyUpdate(){
	...
	NOHUP_EXEC("$cmd >>/usr/share/artica-postfix/ressources/logs/Kav4ProxyUpdate");
}
...
function ldap_upload_organization(){
	...
	$session=$_GET["config-file"];
	$unix=new unix();
	$tmpstr=$unix->FILE_TEMP();
	writelogs_framework("executing  ".LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.export-users.php --upload \"$ou\" \"$session\" >$tmpstr 2>&1",__FUNCTION__,__FILE__,__LINE__);
	shell_exec(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.export-users.php --upload \"$ou\" \"$session\" >$tmpstr 2>&1");
	$results=explode("\n",@file_get_contents($tmpstr));
	@unlink($tmpstr);
	echo "<articadatascgi>". base64_encode(serialize($results))."</articadatascgi>";
}
...
function ifconfig_all_ips(){
	$unix=new unix();
	echo "<articadatascgi>". base64_encode(serialize($unix->ifconfig_all_ips()))."</articadatascgi>";
...
function MLDONKEY_RESTART() {
	NOHUP_EXEC("/etc/init.d/artica-postfix restart mldonkey");
}
...
function disk_format_big_partition(){
	...
	$cmd=LOCATE_PHP5_BIN2() ." /usr/share/artica-postfix/exec.system.build-partition.php --full \"{$_GET["dev"]}\" \"{$_GET["label"]}\" \"{$_GET["fs_type"]}\" >{$GLOBALS["LOGSFILES"]} 2>&1";
...
function postfix_hash_aliases(){
	...
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.postfix-multi.php --instance-reconfigure \"{$_GET["hostname"]}\"");
}
...
function postfix_others_values(){
	...
	$nohup=$unix->find_program("nohup");
	$php=$unix->LOCATE_PHP5_BIN();	
	writelogs_framework("postfix_others_values()...",__FUNCTION__,__FILE__,__LINE__);
	...
	if(isset($_GET["hostname"])){
		...
		if($_GET["hostname"]<>"master"){
			shell_exec("$nohup $php /usr/share/artica-postfix/exec.postfix-multi.php --instance-reconfigure \"{$_GET["hostname"]}\" >/usr/share/artica-postfix/ressources/logs/postfix.othervalues.progress.log 2>&1 &");
			return;
		}
	}
	
	$cmd="$nohup $php /usr/share/artica-postfix/exec.postfix.maincf.php --others-values >/usr/share/artica-postfix/ressources/logs/postfix.othervalues.progress.log 2>&1 &";
	writelogs_framework($cmd,__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);
}
function postfix_mime_header_checks(){
	...
	if($_GET["hostname"]<>null){
		if($_GET["hostname"]=="master"){
			$cmd="$nohup $php /usr/share/artica-postfix/exec.postfix.maincf.php --mime-header-checks >/dev/null 2>&1 &";
			writelogs_framework($cmd,__FUNCTION__,__FILE__,__LINE__);
			shell_exec($cmd);
		}else{
...
function zarafa_admin_chock(){
	...
	$cmd="$nohup $zarafa_admin$socket -l >/dev/null 2>&1 &";
	shell_exec($cmd);
	writelogs_framework($cmd,__FUNCTION__,__FILE__,__LINE__);	
...
function zarafa_migrate(){
	$unix=new unix();
	$nohup=$unix->find_program("nohup");	
	$cmd=$nohup." ".LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.zarafa-migrate.php {$_GET["zarafa-migrate"]} >/dev/null 2>&1 &";
	shell_exec($cmd);
	writelogs_framework($cmd,__FUNCTION__,__FILE__,__LINE__);
}
function zarafa_restart_web(){
	NOHUP_EXEC("/etc/init.d/zarafa-web restart");
	NOHUP_EXEC("/etc/init.d/php5-fpm restart");
}

function zarafa_stop_web(){
	NOHUP_EXEC("/etc/init.d/zarafa-web stop");
	NOHUP_EXEC("/etc/init.d/artica-status reload");
	NOHUP_EXEC("/etc/init.d/monit restart");
...
function zarafa_restart(){
	NOHUP_EXEC("/etc/init.d/zarafa-server restart");
}
...
function squidGuardCompile(){
	...
	@chmod($l,0777);
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.squidguard.php --compile >>$l 2>&1");
...
function ChangeHostName(){
	...
	$unix=new unix();
	$unix->CHANGE_HOSTNAME($servername);
	
}

function ClamavUpdate(){
	NOHUP_EXEC("/usr/share/artica-postfix/bin/artica-update --clamav");
}
...
function postfix_single_status(){
	$EnablePostfixMultiInstance=trim(@file_get_contents("/etc/artica-postfix/settings/Daemons/EnablePostfixMultiInstance"));
	if(!is_numeric($EnablePostfixMultiInstance)){$EnablePostfixMultiInstance=0;}
	if($EnablePostfixMultiInstance==1){
			echo "<articadatascgi>". @file_get_contents("/usr/share/artica-postfix/ressources/logs/global.status.ini")."</articadatascgi>";
			return;
	}
...
function postfix_multi_relayhost(){
	...
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.postfix-multi.php --instance-relayhost \"$hostname\"");	
}
...
function postfix_multi_aiguilleuse(){
	...
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.postfix-multi.php --instance-aiguilleuse \"$hostname\"");	
}


function postfix_multi_mime_header_checks(){
	$hostname=$_GET["postfix-multi-mime-header-checks"];
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.postfix-multi.php --mime-header-checks \"$hostname\"");	
}
...
function postfix_single_mynetworks(){
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.postfix.maincf.php --networks");
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.postfix.maincf.php --headers-check");		
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.opendkim.php --networks");
	NOHUP_EXEC("/etc/init.d/milter-greylist restart");
}

function OPENDKIM_WHITELIST_DOMAINS(){
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.opendkim.php --whitelist-domains");
}
...
NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.postfix.maincf.php --luser-relay --reload");
}
...
function SQUID_RESTART_ALL(){
	...
	@unlink($GLOBALS["SQUID_REFRESH_PANEL_STATUS"]);
	writelogs_framework("Building $tmpfile",__FUNCTION__,__FILE__,__LINE__);
	writelogs_framework("Logs stored in {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]}",__FUNCTION__,__FILE__,__LINE__);
	
	if($GLOBALS["VERBOSE"]){echo "php -> $php\n";}
	$f[]="#!/bin/sh";
	$f[]="PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin";
	$f[]="$echo \"Starting: $tmpfile\" >> {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]}";
	$f[]="$chmod 0777 {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]}";
	$f[]="$echo \"5\" > /usr/share/artica-postfix/ressources/logs/squid.restart.progress2";
	$f[]="$chmod 0777 /usr/share/artica-postfix/ressources/logs/squid.restart.progress2";
	
	
	if(isset($_GET["wccp"])){
		$f[]="$echo \"Building Network:\" >> {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]}";
		$f[]="$echo \"6\" > /usr/share/artica-postfix/ressources/logs/squid.restart.progress2";
		
		$f[]="$php /usr/share/artica-postfix/exec.virtuals-ip.php --build --force >> {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]} 2>&1";
		
		$f[]="$echo \"7\" > /usr/share/artica-postfix/ressources/logs/squid.restart.progress2";
		$f[]="$echo \"Restarting Network:\" >> {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]}";
		$f[]="/etc/init.d/artica-ifup start --script=cmd.php/SQUID_RESTART_ALL >> {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]} 2>&1";
		
		$f[]="$echo \"10\" > /usr/share/artica-postfix/ressources/logs/squid.restart.progress2";
		$f[]="$echo \"Building proxy configuration:\" >> {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]}";
		$f[]="$php /usr/share/artica-postfix/exec.squid.php --build --force --reconfigure --framework >> {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]} 2>&1";
	...
	if(isset($_GET["ApplyConfToo"])){
		$nohup=$unix->find_program("nohup");
		$f[]="$echo \"10\" > /usr/share/artica-postfix/ressources/logs/squid.restart.progress2";
		$f[]="/etc/init.d/ufdbcat stop --force >> {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]} 2>&1";
		$f[]="/etc/init.d/artica-status stop --force >> {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]} 2>&1";
		$cmd="$php /usr/share/artica-postfix/exec.squid.php --build --force --reconfigure --framework >> {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]} 2>&1";
		writelogs_framework("$cmd",__FUNCTION__,__FILE__,__LINE__);
		$f[]=$cmd;
		$f[]="/etc/init.d/ufdbcat start >> {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]} 2>&1";
		$f[]="/etc/init.d/artica-status start >> {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]} 2>&1";
		@file_put_contents($GLOBALS["SQUID_REFRESH_PANEL_STATUS"], $cmd."\n");
		
	}		
	
	
	$f[]="$echo \"Restarting proxy service:\" >> {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]}";
	$f[]="/etc/init.d/squid restart --script=".basename(__FILE__)." >> {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]} 2>&1";
	$f[]="$nohup /etc/init.d/dnsmasq restart --force --framework >> {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]} 2>&1 &";
	$f[]=$addon;
	$f[]="$nohup /etc/init.d/artica-framework restart >> {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]} 2>&1 &";
	$f[]="$nohup /etc/init.d/artica-status restart --force >> {$GLOBALS["SQUID_REFRESH_PANEL_STATUS"]} 2>&1 &";
	
	
	$f[]="rm -f $tmpfile";
	@file_put_contents($tmpfile, @implode("\n", $f));
	@chmod($tmpfile,0755);
	writelogs_framework("$nohup $tmpfile >/dev/null 2>&1 &",__FUNCTION__,__FILE__,__LINE__);
	shell_exec("$nohup $tmpfile >/dev/null 2>&1 &");
...
function ApplyDHCPDService(){
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.dhcpd.compile.php");
}
function ApplyBINDService(){
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.dhcpd.compile.php --bind");
}
...
function SQUID_WRAPZAP(){
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.squid.php --wrapzap");
}
function SQUID_WRAPZAP_COMPILE(){
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.squid.php --wrapzap-compile --reload");
}
...
function CROSSROADS_RESTART(){
	NOHUP_EXEC("/etc/init.d/artica-postfix restart crossroads");
}
...
function PROXY_SAVE(){
	NOHUP_EXEC(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.system.php --proxy");
...
function clamd_restart(){
	$unix=new unix();
	$nohup=$unix->find_program("nohup");
	$cmd="$nohup /etc/init.d/clamav-daemon restart >/dev/null 2>&1 &";
	writelogs_framework($cmd,__FUNCTION__,__FILE__,__LINE__);
	shell_exec(trim($cmd));	
	}
...
function RestartApacheSrc(){
	...
	writelogs_framework($cmd,__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);	
	$cmd="$nohup /etc/init.d/artica-apache restart >/dev/null 2>&1 &";
	writelogs_framework($cmd,__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);	
	if(is_file($nginx)){
...
function zarafa_write_license(){
	...
	NOHUP_EXEC("/etc/init.d/artica-postfix restart zarafa");		
}
...
function USB_SCAN_WRITE(){
	...
	$php=$unix->LOCATE_PHP5_BIN();
	$cmdline="$php /usr/share/artica-postfix/exec.usb.scan.write.php";
	if(isset($_GET["tenir"])){
		$cmd="$cmdline --verbose 2>&1";
		writelogs_framework("$cmd",__FUNCTION__,__FILE__,__LINE__);
		exec($cmd,$results);
	...
	$Time=$unix->file_time_min($timeFile);
	if($Time<5){
		writelogs_framework("{$Time}mn, need to wait 5mn,aborting",__FUNCTION__,__FILE__,__LINE__);
		return;
	}
	@unlink($timeFile);
	@file_put_contents($timeFile, time());	
	...
	$nohup=$unix->find_program("nohup");
	$cmd="$nohup $cmdline >/dev/null 2>&1 &";
	writelogs_framework("$cmd",__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);
}
...
function SysSyncPaquages(){
	...
	$unix->THREAD_COMMAND_SET(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.setup-center.php --force");		
