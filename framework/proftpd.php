...
function install_tgz(){
	...
	$GLOBALS["LOGSFILES"]="/usr/share/artica-postfix/ressources/logs/proftpd.install.progress.txt";
	...
	$php5=$unix->LOCATE_PHP5_BIN();
	$nohup=$unix->find_program("nohup");
	$cmd="$nohup $php5 /usr/share/artica-postfix/exec.proftpd.install.php --install {$_GET["key"]} {$_GET["OS"]} >{$GLOBALS["LOGSFILES"]} 2>&1 &";
	writelogs_framework($cmd ,__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);
...
function systemusers(){
	...
	@file_put_contents("/etc/artica-postfix/settings/Daemons/SystemUsers", serialize($ARRAYU));
