...
function restart(){
	...
	$sock->getFrameWork("services.php?mysql-repair-database=yes");
...
function Filllogs(){
	$datas=explode("\n",@file_get_contents("ressources/logs/web/RepairMysql.log"));
	krsort($datas);
	echo @implode("\n", $datas);
}
...
function logs(){
	...
	$t=$_GET["t"];
	$tt=time();
	$datas=@file_get_contents("ressources/logs/web/RepairMysql.log");
	if(strlen($datas)<10){
