...
if($argv[1]=="--install"){$GLOBALS["OUTPUT"]=true;install($argv[2],$argv[3]);die();}
...
function install($filekey=0,$OS){
	...
	if($filekey<>0){
		$sock=new sockets();
		$ArticaTechNetSuricataRepo=unserialize(base64_decode($sock->GET_INFO("ArticaTechNetSuricataRepo")));
		$version=$ArticaTechNetSuricataRepo[$filekey][$OS]["VERSION"];
		$filename=$ArticaTechNetSuricataRepo[$filekey][$OS]["FILENAME"];
		$MD5=$ArticaTechNetSuricataRepo[$filekey][$OS]["MD5"];
		$URL=$ArticaTechNetSuricataRepo[$filekey][$OS]["URL"];
	}
	...
	if(!is_file("/etc/init.d/suricata")){
		system("$php /usr/share/artica-postfix/exec.initslapd.php --suricata");
		system("$php /usr/share/artica-postfix/exec.initslapd.php --bandwidthd");
	...
	system("$rmmod xt_ndpi");
	system("$rmmod pf_ring");
	
	shell_exec("$tar xvf $tmpdir/$filename -C /");
	$nohup=$unix->find_program("nohup");
	build_progress_idb("{installing} 1/4",96);
	system("$nohup $ldconfig >/dev/null 2>&1 &");
	build_progress_idb("{installing} 2/4",96);
	system("$depmod -a");
	build_progress_idb("{installing} 3/4",96);
	system("$modprobe pf_ring transparent_mode=0 min_num_slots=65534");
	build_progress_idb("{installing} 4/4",96);
	system("$modprobe xt_ndpi");


	if($GLOBALS["PROGRESS"]){
		build_progress_idb("{restarting_service}",97);
		system("$php /usr/share/artica-postfix/exec.status.php --suricata >/dev/null");
		system("/etc/init.d/artica-status restart --force");
		build_progress_idb("{restarting_service} 1/3",98);
		system("/etc/init.d/suricata restart");
		build_progress_idb("{restarting_service} 2/3",98);
		system("/etc/init.d/firehol restart");
		build_progress_idb("{restarting_service} 3/3",98);
		system("/etc/init.d/bandwidthd restart");
	}
