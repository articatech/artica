...
function js(){
	...
	$title=$tpl->javascript_parse_text("{ISCSI_share}::{apply}");
...
function FilesLogs(){
	...
	$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/web/system_disks_iscsi_progress.txt";
...
if($prc==100){
	...
	function Start$time(){
		...
		$('#progress-$t').progressbar({ value: $prc });
		RTMMailHide();
		if(document.getElementById('iscsi-status')){LoadAjax('iscsi-status','system.disks.iscsi.php?iscsi-status=yes');}
...
function popup(){
	...
	$sock->getFrameWork("iscsi.php?build-server-config=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{ISCSI_share}");
