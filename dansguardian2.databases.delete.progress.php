...
function title(){
	...
	$tpl=new templates();
	$sock=new sockets();
	return $tpl->_ENGINE_parse_body("{webfiltering} {delete_databases}");
}
...
function js(){
	...
	$t=time();
	$tpl=new templates();
	$compile_squid_ask=$tpl->javascript_parse_text("{webfiltering} {delete_databases}");
...
function Launch(){
	$sock=new sockets();
	$cmd="squid.php?delete-webfiltering-db=yes";
...
function popup(){
	...
	$title="{webfiltering} {delete_databases}";
