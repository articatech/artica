...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/exec.monit.progress";
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/exec.monit.progress.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{restarting_service} {APP_MONIT}");
	echo "
	YahooWinBrowseHide();		
...
if($prc==100){
	...
	function Start$time(){
		...
		document.getElementById('title-$t').innerHTML='$title';
		$('#progress-$t').progressbar({ value: $prc });
		LoadAjaxRound('health-monit','system.monit.health.php');
		RTMMailHide();
...
function popup(){
	...
	$text=$tpl->_ENGINE_parse_body("{restarting_service} {APP_MONIT}");
