...
if($argv[1]=="--transaction-find"){transaction_find($argv[2],$argv[3]);die();exit;}
start();
function start(){
...
function transaction_find($msgid,$id){
	...
	$q->QUERY_SQL($sql,"artica_events");
	if(!$q->ok){system_admin_events("Fatal, search transaction $msgid failed $q->mysql_error", __FUNCTION__, __FILE__, __LINE__, "postfix");}
}
