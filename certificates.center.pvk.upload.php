...
function certificate_upload_js(){
	...
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("{import_pvk_file}:{upload}:{$_GET["CommonName"]}");
...
function certificate_upload_popup(){
	...
	$UploadAFile=$tpl->javascript_parse_text("{import_pvk_file} - *.pvk");
...
function certificate_upload_save(){
	...
	if(!$q->FIELD_EXISTS("sslcertificates","pvk_content","artica_backup")){$sql="ALTER TABLE `sslcertificates` ADD `pvk_content` TEXT NOT NULL";$q->QUERY_SQL($sql,'artica_backup');}
	$sql="UPDATE sslcertificates SET `pvk_content`='$certificate_content' WHERE CommonName='$CommonName'";
	$q->QUERY_SQL($sql,"artica_backup");
	if(!$q->ok){echo $q->mysql_error."\n";return;}
	$CommonNameEnc=urlencode($CommonName);
	$sock=new sockets();
	echo base64_decode($sock->getFrameWork("openssl.php?pvk-convert=$CommonNameEnc"));
