<?php
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/web/openvpn.client.progress";
...
function title(){
	$tpl=new templates();
	return $tpl->javascript_parse_text("{BUILD_OPENVPN_CLIENT_CONFIG}");
}
...
if($prc==100){
	...
	function Start$time(){
		...
		$('#progress-$t').progressbar({ value: $prc });
		RTMMailHide();
		if(document.getElementById('OPENVPN_CLIENT_SCRIPTS')){
...
function Launch(){
	$sock=new sockets();
	$cmd="openvpn.php?build-vpn-user={$_GET["uid"]}";
...
function popup(){
	...
	$title="{BUILD_OPENVPN_CLIENT_CONFIG}";
