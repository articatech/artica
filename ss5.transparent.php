...
var xAdd$t= function (obj) {
	...
	$('#SS5_TRANSPARENT_RULES').flexReload();
}
function Add$t(){
...
function delete(){
	...
	$sql="DELETE FROM `ss5_transparent` WHERE `ID`='{$_POST["delete"]}'";
	$ligne=@mysql_fetch_array($q->QUERY_SQL($sql,"artica_backup"));
	if(!$q->ok){echo $q->mysql_error;}
}	
...
function table(){
	...
	if(!is_file("/usr/bin/redsocks")){
		echo FATAL_ERROR_SHOW_128("{ERROR_MISSING_MODULE_UPDATE_PROXY}");
		return;
	}
	...
	$sql="CREATE TABLE IF NOT EXISTS `ss5_transparent` (
	...
	$allowdeny=$tpl->javascript_parse_text("{proxy}/{direct}");
	...
	$title=$tpl->javascript_parse_text("{APP_SS5} {transparent_rules}");
	...
	$html="
	<table class='SS5_TRANSPARENT_RULES' style='display: none' id='SS5_TRANSPARENT_RULES' style='width:99%'></table>
	<script>
	var IptableRow='';
	$(document).ready(function(){
	$('#SS5_TRANSPARENT_RULES').flexigrid({
	url: '$page?search=yes',
	dataType: 'json',
	colModel : [
	{display: '<span style=font-size:18px>&nbsp;</span>', name : 'zorder', width :137, sortable : true, align: 'center'},
	{display: '<span style=font-size:18px>$source</span>', name : 'src_host', width :210, sortable : true, align: 'left'},
	{display: '<span style=font-size:18px>$destination</span>', name : 'dst_host', width :210, sortable : true, align: 'left'},
...
function ID_POPUP(){
	...
	if($ID>0){
		$button_name="{apply}";
		$sql="SELECT * FROM ss5_transparent WHERE ID=$ID";
	...
	<tr>
		...
		<td style='font-size:20px'>". Field_array_Hash($array, "eth-$t",$ligne["eth"],"style:font-size:20px;font-wieght:bold")."</td>
		<td>&nbsp;</td>
	</tr>	
	<tr>
...
function SaveRule(){
	...
	$insert="INSERT IGNORE INTO `ss5_transparent` (".@implode(",", $addF).") VALUES (".@implode(",", $addV).")";
	$edit="UPDATE `ss5_transparent` SET ".@implode(",", $editF)." WHERE ID='$ID'";
	$q=new mysql_squid_builder();
	...
	if(!$q->FIELD_EXISTS("ss5_transparent", "eth")){
		$q->QUERY_SQL("ALTER TABLE `ss5_transparent` ADD `eth` VARCHAR(40)");
		if(!$q->ok){echo $q->mysql_error."\n$sql";return;}
	}
	...
	*/
		...
		$data['rows'][] = array(
				...
				'cell' => array(
						...
						"<span style='font-size:22px'>$link{$ligne["src_host"]}</a></span>",
