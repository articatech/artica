...
function popup_js(){
	...
	$q=new mysql();
	$sql="SELECT pattern FROM spamasssin_subjects WHERE `ID`='{$_GET["ID"]}'";
...
function popup_add(){
	...
	<div class=explain style='font-size:18px'>{spamassassin_subjects_explain}</div>
...
<script>
	var xSave$t= function (obj) {
		...
		YahooWin3Hide();
		$('#SPAMASSASSIN_SUBJECTS_TABLE').flexReload();
	}
...
function SaveItems(){
	...
	while (list ($num, $ligne) = each ($items2) ){
		...
		$q=new mysql();
		$q->QUERY_SQL("INSERT IGNORE INTO spamasssin_subjects (pattern,zdate) VALUE ('$ligne',NOW())","artica_backup");
...
function SaveItemID(){
	...
	$q=new mysql();
	$q->QUERY_SQL("UPDATE spamasssin_subjects SET pattern='$items' WHERE ID='{$_POST["ID"]}'","artica_backup");
	if(!$q->ok){echo $q->mysql_error;return;}
}
...
function remove(){
	$q=new mysql();
	$q->QUERY_SQL("DELETE FROM spamasssin_subjects WHERE ID='{$_POST["remove"]}'","artica_backup");
...
function table(){
	...
	$title=$tpl->javascript_parse_text("{subject_rules}");
...
$buttons="
	buttons : [
		...
		$html="
		<table class='SPAMASSASSIN_SUBJECTS_TABLE' style='display: none' id='SPAMASSASSIN_SUBJECTS_TABLE' style='width:100%'></table>
		<script>
		var xsite='';
		$(document).ready(function(){
		$('#SPAMASSASSIN_SUBJECTS_TABLE').flexigrid({
...
var xRemoveSpamAssassineSubjects= function (obj) {
	var tempvalue=obj.responseText;
...
function RemoveSpamAssassineSubjects(ID){
	var XHR = new XHRConnection();
	XHR.appendData('remove',ID);
	XHR.sendAndLoad('$page', 'POST',xRemoveSpamAssassineSubjects);
	}
...
function search(){
	...
	if($sql_search<>null){
		...
		$sql="SELECT COUNT(*) AS TCOUNT FROM spamasssin_subjects 1 $sql_search";
		$ligne=mysql_fetch_array($q->QUERY_SQL($sql,"artica_backup"));
		$total = $ligne["tcount"];
	
	}else{
		$sql="SELECT COUNT(*) AS tcount FROM spamasssin_subjects";
		$ligne=mysql_fetch_array($q->QUERY_SQL($sql,"artica_backup"));
		$total = $ligne["tcount"];
	}
	...
	$sql="SELECT * FROM spamasssin_subjects WHERE 1 $sql_search $ORDER $limitSql";
	...
	while ($ligne = mysql_fetch_assoc($results)) {
		...
		$delete=imgsimple("delete-42.png",null,"RemoveSpamAssassineSubjects($ID)");
