...
$GLOBALS["CACHEFILE"]="/usr/share/artica-postfix/ressources/logs/web/squid.windowsupdate1.progress";
...
function title(){
	...
	$tpl=new templates();
	$sock=new sockets();
	return $tpl->_ENGINE_parse_body("{windows_updates_cache_enforcement}");
}
...
if($prc==100){
	...
	function Start$time(){
		...
		document.getElementById('title-$t').innerHTML='$title';
		$('#progress-$t').progressbar({ value: $prc });
		RefreshTab('main_windowsupdate_tabs');
		RTMMailHide();
		CacheOff();
	}
	setTimeout(\"Start$time()\",1000);
...
function Launch(){
	$sock=new sockets();
	$cmd="windowsupdate.php?build=yes";
...
function popup(){
	...
	$title="{windows_updates_cache_enforcement}";
