...
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/web/freeweb.rebuild.progress.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{rebuild} {$_GET["servername"]}");
	echo "
	YahooWinBrowseHide();		
	RTMMail('800','$page?popup=yes&servername=".urlencode($_GET["servername"])."','$title');";
...
if($md5file<>$_GET["md5file"]){
	...
	var xStart$time= function (obj) {
		...
		Loadjs('$page?build-js=yes&t=$t&md5file=$md5file&servername=".urlencode($_GET["servername"])."');
	}		
...
function popup(){
	...
	$sock->getFrameWork("freeweb.php?rebuild-vhost=yes&servername=".urlencode($_GET["servername"])."");
	$t=$_GET["t"];
