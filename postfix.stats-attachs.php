...
function requeteur_js(){
	...
	$page=CurrentPageName();
	$tpl=new templates();
	$build_the_query=$tpl->javascript_parse_text("{build_the_query}::{attachments}");
...
function build_query_js(){
	...
	$t=$_GET["t"];
	$user=$_GET["user"];
	$md5=md5("SMTP_ATTACHS:$from$to$interval$user{$_GET["searchsites"]}{$_GET["searchuser"]}{$_SESSION["uid"]}");
	...
	$nextFunction="LoadAjax('SMTP_ATTACHS_STATS_MAIN_GRAPH','$page?main=yes&zmd5=$md5&t=$t');";
	...
	if(intval($ligne["ID"])==0){
		...
		$title="{attachments}: $timetext1 - {to} $timetext2 email {like} {$_GET["searchuser"]}";
		$sql="INSERT IGNORE INTO `reports_cache` (`zmd5`,`title`,`report_type`,`zDate`,`params`,`ou`,`uid`) VALUES 
		('$md5','$title','SMTP_ATTACHS',NOW(),'$serialize','$ou','$uid')";
		$q->QUERY_SQL($sql);
...
function page(){
	...
	if($_GET["zmd5"]==null){
		$q=new mysql_squid_builder();
		$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT title,zmd5 FROM reports_cache WHERE report_type='SMTP_ATTACHS' ORDER BY zDate DESC LIMIT 0,1"));
	}else{
		$q=new mysql_squid_builder();
	...
	if($ligne["zmd5"]<>null){
		$nextFunction="LoadAjax('SMTP_ATTACHS_STATS_MAIN_GRAPH','$page?main=yes&zmd5={$ligne["zmd5"]}&t=$t');";
		$content=null;
	...
	$html="
	<div id='SMTP_ATTACHS_STATS_MAIN_GRAPH'>$content</div>
	<script>
...
function graph2(){
	...
	$results=$q->QUERY_SQL("SELECT SUM(size) as size,ext FROM \"$table\" GROUP BY ext ORDER BY size desc LIMIT 15");
	while($ligne=@pg_fetch_assoc($results)){
		$size=$ligne["size"];
		$size=$size/1024;
		$size=$size/1024;
	...
	$highcharts->PiePlotTitle="{top} {extensions}";
	$highcharts->Title=$tpl->_ENGINE_parse_body("TOP {extensions}/{size}");
	echo $highcharts->BuildChart();
	echo "\n";
	echo "if(document.getElementById('websites-button-area')){document.getElementById('websites-button-area').innerHTML='';}\n";
...
function table2(){
	...
	$html[]=$tpl->_ENGINE_parse_body("<tr><th style='font-size:18px;padding:8px'>{top} {content_type}/{size}</td>
			<th style='font-size:18px'>{size}</td></tr>");
	
	$results=$q->QUERY_SQL("SELECT SUM(size) as size,contenttype FROM \"$table\" GROUP BY contenttype ORDER BY size desc LIMIT 15");
	while($ligne=@pg_fetch_assoc($results)){
		$site=$ligne["contenttype"];
		
		$length=strlen($site);
		if($length>50){
			$site=texttooltip(substr($site,0,47)."...",$site);
		}
...
function graph3(){
	...
	$q=new postgres_sql();
	$results=$q->QUERY_SQL("SELECT SUM(hits) as hits,fname FROM \"$table\" GROUP BY fname ORDER BY hits DESC LIMIT 10");
	...
	$highcharts->PieDatas=$PieData;
	$highcharts->ChartType="pie";
	$highcharts->PiePlotTitle="{top} {filename}";
	$highcharts->Title=$tpl->_ENGINE_parse_body("TOP {filename}/{hits}");
	echo $highcharts->BuildChart();
	echo "\n";
	echo "if(document.getElementById('websites-button-area')){document.getElementById('websites-button-area').innerHTML='';}\n";
...
function table3(){
	...
	$html[]="<table style='width:100%'>";
	$html[]=$tpl->_ENGINE_parse_body("<tr><th style='font-size:18px;padding:8px'>{filename}</td><th style='font-size:18px'>{hits}</td></tr>");
	while($ligne=@pg_fetch_assoc($results)){
		$size=FormatNumber($ligne["hits"]);
		$site=$ligne["fname"];
...
function table1(){
	...
	$html[]="<table style='width:100%'>";
	$html[]=$tpl->_ENGINE_parse_body("<tr><th style='font-size:18px;padding:8px'>{top} {sender}</td><th style='font-size:18px'>{attachments}</td></tr>");
	...
	while($ligne=@pg_fetch_assoc($results)){
		...
		$length=strlen($mailfrom);
		if($length>50){
			$mailfrom=texttooltip(substr($mailfrom,0,47)."...",$mailfrom);
		}
