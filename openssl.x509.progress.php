...
function title(){
	$tpl=new templates();
	return $tpl->javascript_parse_text("{certificate} {$_GET["generate-x509"]}");
}


function suffix(){
	return "&generate-x509=".urlencode($_GET["generate-x509"]);
}
...
function Launch(){
	$sock=new sockets();
	$cmd="openssl.php?generate-x509=".urlencode($_GET["generate-x509"]);
