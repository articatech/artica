...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/proftpd.install.progress";
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/proftpd.install.progress.txt";
...
function js(){
	...
	if($_GET["key"]<>null){
		$sock=new sockets();
		$ArticaTechNetSquidRepo=unserialize(base64_decode($sock->GET_INFO("ArticaTechNetProFTPDRepo")));
...
if($prc==100){
	...
	function Start$time(){
		...
		LoadAjaxRound('proftpd-update','proftpd.update.php');
		LayersTabsAllAfter();
		RTMMailHide();
		CacheOff();
		GotoVSFTPD();
...
function popup(){
	...
	$sock->getFrameWork("proftpd.php?install-tgz=".urlencode($_GET["filename"])."&key={$_GET["key"]}&OS={$_GET["OS"]}");
	$t=$_GET["t"];
