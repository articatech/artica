...
function js(){
	...
	$t=time();
	$title=$tpl->javascript_parse_text("{reconfigure_hostpot_service}");
	$ask=$tpl->javascript_parse_text("{WARN_RESTART_HOTSPOT}");
	echo "
	function Start$t(){	
...
function popup(){
	...
	$page=CurrentPageName();
	$sock=new sockets();
	$sock->getFrameWork("hotspot.php?reconfigure-progress=yes");
	$t=$_GET["t"];
