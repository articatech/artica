<?php
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/web/squid.articadb.restore.progress";
...
function title(){
	$tpl=new templates();
	return $tpl->javascript_parse_text("{restore}:");
}
...
function suffix(){
	...
	return "&filename=".urlencode($_REQUEST["filename"]);
...
if($md5file<>$_GET["md5file"]){
	...
	function Start$time(){
		...
		XHR.appendData('filename', '{$_REQUEST["filename"]}');
		XHR.appendData('t', '$t');
		XHR.setLockOff();
...
function Launch(){
	$sock=new sockets();
	$cmd="squid.php?articadb-restore=yes".suffix();
