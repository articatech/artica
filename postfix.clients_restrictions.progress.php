...
function js(){
	...
	$title=$tpl->javascript_parse_text("{smtpd_client_restrictions}");
...
function buildjs(){
	...
	$logsFile="/usr/share/artica-postfix/ressources/logs/smtpd_client_restrictions_progress.log";
	header("content-type: application/x-javascript");
	$tpl=new templates();
	$page=CurrentPageName();
...
function popup(){
	...
	$text=$tpl->_ENGINE_parse_body("{smtpd_client_restrictions}<br>{please_wait_preparing_settings}...");
