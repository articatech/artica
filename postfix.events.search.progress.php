...
function js(){
	...
	$title=$tpl->javascript_parse_text("{history_search}");
...
if($prc==100){
	...
	function Start$time(){
		...
		$('#progress-$t').progressbar({ value: $prc });
		RTMMailHide();
		$('#POSTFIX_LOG_HISTORY').flexReload();
...
function popup(){
	...
	$sock->getFrameWork("postfix2.php?history-search=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{history_search}...");
