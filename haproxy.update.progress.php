...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/haproxy.install.progress";
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/haproxy.install.progress.txt";
...
if($prc==100){
	...
	function Start$time(){
		...
		document.getElementById('title-$t').innerHTML='$title';
		$('#progress-$t').progressbar({ value: $prc });
		LoadAjaxRound('Haproxy-Updates','haproxy.update.php');
		LayersTabsAllAfter();
		RTMMailHide();
		CacheOff();
		GotToHAPROXY();
...
function popup(){
	...
	$sock->getFrameWork("haproxy.php?install-tgz=".urlencode($_GET["filename"])."&key={$_GET["key"]}&OS={$_GET["OS"]}");
	$t=$_GET["t"];
