...
function js(){
	...
	if($EnableArticaHotSpot==0){die();}
	if($EnableMaintenanceHotSpot==0){die();}
	...
	$t=time();
	$title=$tpl->javascript_parse_text("{disable_emergency_mode}");
...
if($prc==100){
	...
	function Start$time(){
		...
		document.getElementById('title-$t').innerHTML='$title';
		$('#progress-$t').progressbar({ value: $prc });
		if(document.getElementById('layer-hotspot-maintenance')){
			YahooWinHide();
			Loadjs('squid.webauth.maintenance.php');
		}
		
		if(document.getElementById('squid_hotspot')){
...
function popup(){
	...
	$page=CurrentPageName();
	$sock=new sockets();
	$sock->getFrameWork("hotspot.php?emergency-off=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{disable_emergency_mode}<br>{please_wait_preparing_settings}...");
