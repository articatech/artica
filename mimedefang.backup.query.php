...
function delete_message_js(){
	...
	$q=new postgres_sql();
	$ligne=pg_fetch_array($q->QUERY_SQL("SELECT * FROM backupmsg WHERE id='{$_GET["delete-message-js"]}'"));
...
var xAdd$t= function (obj) {
	...
	$('#BACKUP_MESSAGES_TABLE').flexReload();
}
function Add$t(){
...
function delete_message(){
	...
	$tpl=new templates();
	$q=new postgres_sql();
	$ligne=pg_fetch_array($q->QUERY_SQL("SELECT msgmd5 FROM backupmsg WHERE id='{$_POST["delete-message"]}'"));
	$msgmd5=$ligne["msgmd5"];
	
	$ligne=pg_fetch_array($q->QUERY_SQL("SELECT contentid FROM backupdata WHERE msgmd5='$msgmd5'"));
...
function popup(){
	...
	$title=$tpl->_ENGINE_parse_body("&laquo;{backupemail_behavior}&raquo;");
	...
	$html="
	<table class='BACKUP_MESSAGES_TABLE' style='display: none' id='BACKUP_MESSAGES_TABLE' style='width:99%'></table>
<script>
var mem$t='';
$(document).ready(function(){
$('#BACKUP_MESSAGES_TABLE').flexigrid({
...
function items(){
	...
	if(!$users->AsPostfixAdministrator){
		if($users->AsMessagingOrg){
			...
			$table="(select * FROM backupmsg WHERE ($imploded1) OR ($imploded2)) as t";
		}
