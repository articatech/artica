...
function title(){
	...
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("HotSpot::{activate_hostpot}");
	return $title;
}
...
if($prc==100){
	...
	function Start$time(){
		...
		RTMMailHide();
		CacheOff();
		LoadAjaxRound('hostspot-v3','squid.webauth.php?tabs=yes');
		}
	setTimeout(\"Start$time()\",1000);
	";	
	return;	
...
function Launch(){
	...
	$cmd="hotspot.php?wizard-progress=yes";
