...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/squid.quotasband.status.progress";
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/squid.quotasband.status.progress.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{limit_rate}: {status}");
...
function popup(){
	...
	$sock->getFrameWork("squid2.php?quotarules-status-progress=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{limit_rate}: {status}...");
