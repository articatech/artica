...
function js(){
	...
	$title=$tpl->javascript_parse_text("{APP_CLAMAV_MILTER}");
...
function buildjs(){
	...
	$time=time();
	$MEPOST=0;
	$cachefile="/usr/share/artica-postfix/ressources/logs/APP_CLAMAV_MILTER.progress";
	$logsFile="/usr/share/artica-postfix/ressources/logs/APP_CLAMAV_MILTER.log";
	header("content-type: application/x-javascript");
	$tpl=new templates();
	$page=CurrentPageName();
...
function popup(){
	...
	$restart=null;
	if($_GET["restart"]=="yes"){$restart="&restart=yes"; }
	$sock->getFrameWork("postfix.php?clamav-milter=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{APP_CLAMAV_MILTER}<br>{please_wait_preparing_settings}...");
