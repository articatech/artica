...
$GLOBALS["CACHEFILE"]="/usr/share/artica-postfix/ressources/logs/web/mimedefang.resend.progress.{$_REQUEST["id"]}";
$GLOBALS["LOGSFILES"]="/usr/share/artica-postfix/ressources/logs/web/mimedefang.resend.progress.{$_REQUEST["id"]}.log";
...
function title(){
	$tpl=new templates();
	$q=new postgres_sql();
	$ligne=pg_fetch_array($q->QUERY_SQL("SELECT subject FROM quarmsg WHERE id='{$_GET["id"]}'"));
	$subject=$ligne["subject"];
	return $tpl->_ENGINE_parse_body("{resend} $subject");
}
...
function Launch(){
	$sock=new sockets();
	$cmd="mimedefang.php?resend-quarantine={$_GET["id"]}";
