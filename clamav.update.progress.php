...
function js(){
	...
	$t=time();
	$title=$tpl->javascript_parse_text("{udate_clamav_databases}");
...
function popup(){
	...
	$page=CurrentPageName();
	$sock=new sockets();
	$sock->getFrameWork("clamav.php?freshclam-progress=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{udate_clamav_databases}...");
