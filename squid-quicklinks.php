...
if($_GET["stats"]){
	...
	if($DisableArticaProxyStatistics==0){
		if($StatsPerfsSquidAnswered==0){$CPU=$users->CPU_NUMBER;$MEM=$users->MEM_TOTAL_INSTALLEE;if(($CPU<4) AND (($MEM<3096088))){WARN_SQUID_STATS();die();}}}
	}
...
$tr2[]=$tpl->_ENGINE_parse_body(quicklinks_paragraphe("web-site-48.png", "main_interface","main_interface_back_interface_text", "QuickLinksHideUsrProxy()"));
...
function blocked_statistics(){
	...
	$DisableArticaProxyStatistics=$sock->GET_INFO("DisableArticaProxyStatistics");
	if(!is_numeric($DisableArticaProxyStatistics)){$DisableArticaProxyStatistics=0;}
	if($DisableArticaProxyStatistics==1){traffic_statistics_disabled();return;}	
	
	$html="<div id='squid_blocked_stats'></div>
	
	<script>
		LoadAjax('squid_blocked_stats','squid.blocked.statistics.php');
		QuickLinkShow('quicklinks-blocked_websites');
	</script>
...
function section_proxy_events_popup(){
	echo "<script>s_PopUp('squid.accesslogs.php?external=yes',1024,768);</script>";
}

function section_webfiltering_dansguardian(){
	
	echo "<script>LoadAjax('BodyContent','squid.main.quicklinks.php?function=section_webfiltering_dansguardian');QuickLinkShow('quicklinks-WEB_FILTERING');</script>";
	
}

function section_parameters_statapp(){
	echo "<script>LoadAjax('BodyContent','squid.stats-appliance-master.php?inline=yes');QuickLinkShow('quicklinks-parameters');</script>";
	
}

function traffic_statistics_disabled(){
	$tpl=new templates();
	$html="<table style='width:99%' class=form>
	<tr>
		<td width=1%><img src='img/report-warning-256.png'></td>
		<td width=99% style='font-size:18px'>{DisableArticaProxyStatistics_warn}</td>
	</tr>
	</table>
	";
	echo $tpl->_ENGINE_parse_body($html);
...
function WARN_SQUID_STATS(){
	$t=time();
	$html="
	<div id='$t'></div>
	<script>
		LoadAjax('$t','squid.warn.statistics.php');
	</script>
	";
	echo $html;
