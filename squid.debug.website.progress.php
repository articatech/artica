...
function js(){
	...
	$t=time();
	$title=$tpl->javascript_parse_text("{website_analysis}");
	echo "
	function Start$t(){	
...
if($prc==100){
	...
	function Start$time(){
		...
		document.getElementById('title-$t').innerHTML='$title';
		$('#progress-$t').progressbar({ value: $prc });
		LoadAjax('report-WebSiteAnalysis','squid.debug.php?website-analysis-report=yes&t=$t');
		RTMMailHide();
		}
...
function popup(){
	...
	$text=$tpl->_ENGINE_parse_body("{website_analysis}: {please_wait_preparing_settings}...");
