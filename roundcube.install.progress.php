<?php
	$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/roundcube.install.progress";
	$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/web/roundcube.install.progress.txt";
...
function title(){
	...
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("{install_now}: RoundCube");
	return $title;
}
...
if($prc==100){
	...
	function Start$time(){
		...
		RTMMailHide();
		CacheOff();
		LoadAjaxRound('roundcube-section','roundcube.index.php?ajax-pop=yes');
		}
	setTimeout(\"Start$time()\",1000);
	";	
	return;	
...
function Launch(){
	...
	$cmd="roundcube.php?install-progress=yes";
...
function popup(){
	...
	$page=CurrentPageName();
	Launch();
	$text=$tpl->_ENGINE_parse_body("{install_now}: RoundCube");
	$t=time();
