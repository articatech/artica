...
function title(){
	...
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("{APP_SQUID}::{squid_logrotate_perform}");
	return $title;
}
...
if($prc==100){
	...
	function Start$time(){
		...
		$('#progress-$t').progressbar({ value: $prc });
		RTMMailHide();
		if(document.getElementById('main_artica_enforce_rules')){
			RefreshTab('main_artica_enforce_rules');
			return;
		}
		
		if(document.getElementById('main_cache_rules_main_tabs')){
...
function Launch(){
	$sock=new sockets();
	$cmd="squid.php?rotate-progress=yes";
