...
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/web/squid.ad.disconnect.progress.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{disconnect_from_activedirectory}");
	$ask=$tpl->javascript_parse_text("{disconnect_from_activedirectory_explain}");
	echo "
	if(confirm('$ask')){
		YahooWinBrowseHide();		
...
function popup(){
	...
	$sock->getFrameWork("squid.php?disconnect-activedirectory-progress=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{disconnect_from_activedirectory}...");
