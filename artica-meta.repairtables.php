...
$GLOBALS["LOGSFILES"]="/usr/share/artica-postfix/ressources/logs/web/artica-meta.RepairTables.log";
...
function title(){
	$tpl=new templates();
	return $tpl->javascript_parse_text("{repair_tables}");
}
...
function suffix(){
	...
	return "&servername=".urlencode($_GET["servername"]);
...
function Launch(){
	$sock=new sockets();
	$cmd="artica.php?meta-repair-tables=yes".suffix();
...
function popup(){
	...
	$title="{PLEASE_WAIT_REPAIR_TABLES}";
