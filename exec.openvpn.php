...
function BuildBridgeServer(){
    ...
    $routess=array();
    if($openvpn==null){
    	echo "Starting......: ".date("H:i:s")." OpenVPN bridge unable to stat openvpn binary\n";
    	@unlink("/etc/openvpn/cmdline.conf");
    	exit;
    }
    
    if($brctl==null){
    	echo "Starting......: ".date("H:i:s")." OpenVPN bridge unable to stat brctl binary\n";
    	@unlink("/etc/openvpn/cmdline.conf");
    	exit;
    }  

    if($ifconfig==null){
    	echo "Starting......: ".date("H:i:s")." OpenVPN bridge unable to stat ifconfig binary\n";
    	@unlink("/etc/openvpn/cmdline.conf");
    	exit;
    }

    if($ip_tools==null){
    	echo "Starting......: ".date("H:i:s")." OpenVPN bridge unable to stat ip binary\n";
    	@unlink("/etc/openvpn/cmdline.conf");
    	exit;
    }      
   ...
   $crt="/etc/artica-postfix/openvpn/keys/vpn-server.crt";    
    ...
    if($array_ip["IPADDR"]==null){
    	echo "Starting......: ".date("H:i:s")." OpenVPN bridge for $BRIDGE_ETH (failed to get IP informations)...\n";	
    	return;
    }
    
if(preg_match("#^(.+?)\.([0-9]+)$#",$array_ip["IPADDR"],$re)){$eth_broadcast=$re[1].".255";}
    echo "Starting......: ".date("H:i:s")." OpenVPN bridge for tap0 -> $original_eth {$array_ip["IPADDR"]}/$eth_broadcast...\n";
    $br0_ip=GetIpaddrOf("br0");
    echo "Starting......: ".date("H:i:s")." OpenVPN bridge for br0=$br0_ip\n";
    
    if($br0_ip==null){
    	echo "Starting......: ".date("H:i:s")." OpenVPN bridge creating tap0\n";
		...
		$br0_ip=GetIpaddrOf("br0");  
		if($br0_ip==null){
			   echo "Starting......: ".date("H:i:s")." OpenVPN failed to create bridge rolling back\n";
			   StopServer();
			   return; 
		}
...
function BuildBridgeServer_eth_infos($BRIDGE_ETH){
		...
		if(!$q->ok){
			echo "Starting......: ".date("H:i:s")." OpenVPN mysql error $q->mysql_error (". __FUNCTION__.")\n";	
		}
		if($ligne["ipaddr"]==null){
			echo "Starting......: ".date("H:i:s")." OpenVPN $BRIDGE_ETH (has $eth ID={$re[2]}) has no information\n";
		}
...
function GetRoutes(){
   ...
   if($ini->_params["GLOBAL"]["ENABLE_BRIDGE_MODE"]==1){
   		$BRIDGE_ETH=$ini->_params["GLOBAL"]["BRIDGE_ETH"];
   		$array_ip=BuildBridgeServer_eth_infos($BRIDGE_ETH);
   		echo "Starting......: ".date("H:i:s")." OpenVPN binding $BRIDGE_ETH to \"{$array_ip["IPADDR"]}/{$array_ip["NETMASK"]}\"\n";
   		if(preg_match("#(.+?)\.[0-9]+$#",trim($array_ip["IPADDR"]),$re)){
   			$LOCAL_ROUTE="{$re[1]}.0 {$array_ip["NETMASK"]}";
   			$LOCAL_ROUTE=str_replace("Array","",$LOCAL_ROUTE);
   		    echo "Starting......: ".date("H:i:s")." OpenVPN default Local route \"$LOCAL_ROUTE\"\n";
...
if(count($cleaned_routes)>0){ 
	...
	while (list ($a, $b) = each ($cleaned_routes) ){
		 echo "Starting......: ".date("H:i:s")." OpenVPN route $b\n";
		$c[]=$b;}
...
function CheckLzo(){
	$usrs=new usersMenus();
	if($usrs->ArchStruct<>64){return;}
	if(!is_file("/usr/bin/apt-get")){return;}
	if(is_file("/usr/lib/x86_64-linux-gnu/liblzo2.so.2")){return;}
	shell_exec("/usr/bin/apt-get install -f --force-yes -q install liblzo2-2");
...
LoadArgvs();
  ...
  if(preg_match("#^(.+?)\.#",$servername,$re)){$servername=$re[1];}
   $servername=strtoupper($servername);       
   echo "Starting......: ".date("H:i:s")." OpenVPN building settings for $servername...\n";
   ...
   if($ini->_params["GLOBAL"]["ENABLE_BRIDGE_MODE"]==1){
   		echo "Starting......: ".date("H:i:s")." OpenVPN building settings mode bridge enabled...\n";
   		BuildBridgeServer();
   		return;
   }
...
if($IPTABLES_ETH<>null){
		echo "Starting......: ".date("H:i:s")." OpenVPN linked to $IPTABLES_ETH ({$ethi[$IPTABLES_ETH]})...\n";
		if($IPTABLES_ETH<>"127.0.0.1"){
			$IPTABLES_ETH_ROUTE=IpCalcRoute($ethi[$IPTABLES_ETH]);
		}
}else{
	echo "Starting......: ".date("H:i:s")." OpenVPN no local NIC linked...\n";
}
...
if(count($routess)==0){
	if($IPTABLES_ETH_ROUTE<>null){
		if($IPTABLES_ETH<>"127.0.0.1"){
			if($REMOVE_SERVER_DEFAULT_ROUTE==0){
				echo "Starting......: ".date("H:i:s")." OpenVPN IP adding default route \"$IPTABLES_ETH_ROUTE\"\n";
				$routess[]="--push \"route $IPTABLES_ETH_ROUTE\"";
			}
		}
	}
  }else{
  	echo "Starting......: ".date("H:i:s")." OpenVPN IP adding ".count($routess)." routes\n";
  }
   ...
   if(trim($bind_addr)<>null){
   	...
   	echo "Starting......: ".date("H:i:s")." OpenVPN IP bind $bind_addr\n";
   }
   ...
   if(preg_match("#(.+?)\.([0-9]+)$#",$IP_START,$re)){
   	...
   	echo "Starting......: ".date("H:i:s")." OpenVPN IP pool from {$re[1]}.2 to {$re[1]}.254 mask:$NETMASK\n";
   	$server_ip="{$re[1]}.1";
   ...
   if($NETMASK==null){
			...
			if($NETMASK=="255.255.255.255"){$NETMASK="255.255.255.0";}		
   			echo "Starting......: ".date("H:i:s")." OpenVPN Netmask is null for the range $calc_ip, assume $NETMASK\n";
	...
	if($OpenVpnPasswordCert==null){$OpenVpnPasswordCert="MyKey";}
 	...
 	if(isset($GLOBALS["OPENVPNPARAMS"]["duplicate-cn"])){
 		echo "Starting......: ".date("H:i:s")." OpenVPN duplicate-cn is enabled\n";
 		$duplicate_cn=" --duplicate-cn ";
 		$ifconfig_pool_persist=null;
 	}
 	
 	if(isset($GLOBALS["OPENVPNPARAMS"]["iproute"])){
 		echo "Starting......: ".date("H:i:s")." OpenVPN iproute is enabled\n";
 		$ip=$unix->find_program("ip");
 		$iproute=" --iproute $ip";
 	}
 	
 	if(isset($GLOBALS["OPENVPNPARAMS"]["script-security"])){
 		echo "Starting......: ".date("H:i:s")." OpenVPN script-security is enabled\n";
 		$script_security=" --script-security 2";
 	} 	
 	
	if(!is_dir("/etc/openvpn/cdd")){@mkdir("/etc/openvpn/cdd");}
	$already=array();
 	echo "Starting......: ".date("H:i:s")." OpenVPN get remote sites routes...\n";
 	$sql="SELECT sitename,IP_START,netmask,remote_site_routes,FixedIPAddr FROM vpnclient WHERE connexion_type=1";
 	$q=new mysql();
	$results=$q->QUERY_SQL($sql,"artica_backup");
	if(!$q->ok){echo "Starting......: ".date("H:i:s")." OpenVPN : $q->mysql_error\n";}
 	while($ligne=mysql_fetch_array($results,MYSQL_ASSOC)){
 		...
 		if($IP_START_PREFIX<>null){
 			if($FixedIPAddr>2){
 				if($FixedIPAddr<255){
 					echo "Starting......: ".date("H:i:s")." OpenVPN $sitename $IP_START_PREFIX.$FixedIPAddr fixed IP address\n";
 		...
 		if(!isset($already[$ligne["IP_START"]])){
 			echo "Starting......: ".date("H:i:s")." OpenVPN $sitename ({$ligne["IP_START"]} {$ligne["netmask"]})\n";
 		...
 		if(is_array($remote_site_routes)){
			while (list ($num, $site_mask) = each ($remote_site_routes) ){
				if(!isset($already[$num])){
					echo "Starting......: ".date("H:i:s")." OpenVPN $sitename ($num $site_mask)\n";
 		...
 		echo "Starting......: ".date("H:i:s")." OpenVPN cdd $sitename\n";
 	...
 	if($LDAP_AUTH==1){
 		...
 		echo "Starting......: ".date("H:i:s")." OpenVPN auth is enabled\n";
   ...
   echo "Starting......: ".date("H:i:s")." OpenVPN building settings done.\n";
...
function FIX_IP_START($ip,$local){
	...
	if($ip==$local){
		...
		echo "Starting......: ".date("H:i:s")." OpenVPN bad server parameters $original_ip.\n";
		echo "Starting......: ".date("H:i:s")." OpenVPN vpn dhcp parameters ($ip) must no reflect local network ($local).\n";
		echo "Starting......: ".date("H:i:s")." OpenVPN change automatically to $ip_1.$ip_2.$ip_3.0.\n";
		$ip="$ip_1.$ip_2.$ip_3.0";
	}
	...
	if($ip=="255.255.255.0"){
		...
		if("10.8.0.0"<>$local){
			echo "Starting......: ".date("H:i:s")." OpenVPN $ip seems to be a netmask not an IP address change to 10.8.0.0\n";
			return "10.8.0.0";
		}
	}
...
if($GLOBALS["IPTABLES_ETH"]==null){
	while (list ($num, $ligne) = each ($ethi) ){
		if(preg_match("#^eth[0-9]+#",$num)){
			if($GLOBALS["server-conf"]){echo "Starting......: ".date("H:i:s")." OpenVPN no local NIC linked: assume $num ($ligne)...\n";}
			return $num;
		}
	}
}	
...
function Synchronize_clients(){
	...
	while (list ($num, $id) = each ($tbl) ){
		...
		$id=$re[1];
		echo "Starting......: ".date("H:i:s")." OpenVPN checking client ID:$id\n";
		$id=trim($id);
		$mustkill=false;
...
function BuildOpenVpnClients(){
	...
	$q=new mysql();
	$results=$q->QUERY_SQL($sql,"artica_backup");
	if(!$q->ok){echo "Starting......: ".date("H:i:s")." OpenVPN client, mysql database error, starting from cache\n";return null;}
	...
	while($ligne=mysql_fetch_array($results,MYSQL_ASSOC)){
		...
		echo "Starting......: ".date("H:i:s")." OpenVPN client, building configuration for {$ligne["connexion_name"]}\n";
		...
		if($ligne["EnableAuth"]==1){
			echo "Starting......: ".date("H:i:s")." OpenVPN client, remote authentication is enabled for {$ligne["connexion_name"]}\n";
...
function BuildOpenVpnSingleClient($ID){
	...
	$q=new mysql();	
	$ligne=@mysql_fetch_array($q->QUERY_SQL($sql,"artica_backup"));	
	if(!$q->ok){echo "Starting......: ".date("H:i:s")."  OpenVPN client ID $ID, mysql database error, starting from cache\n";return null;}
	$subpath="$main_path/$ID";
	@mkdir("$subpath",0666,true);
	$password=base64_decode($ligne["keypassword"]);
	if($password==null){$password="MyKey";}
	echo "Starting......: ".date("H:i:s")." OpenVPN client ID $ID, building configuration for {$ligne["connexion_name"]}\n";
	@file_put_contents("$subpath/ca.crt",$ligne["ca_bin"]);
	@file_put_contents("$subpath/certificate.crt",$ligne["cert_bin"]);
	@file_put_contents("$subpath/master-key.key",$ligne["key_bin"]);
	@file_put_contents("$subpath/settings.ovpn",$ligne["ovpn"]);
	...
	@unlink("$subpath/auth-user-pass");
	if($ligne["EnableAuth"]==1){
		echo "Starting......: ".date("H:i:s")." OpenVPN client ID $ID, remote authentication is enabled for {$ligne["connexion_name"]}\n";
		@file_put_contents("$subpath/auth-user-pass","{$ligne["AuthUsername"]}\n{$ligne["AuthPassword"]}");
	}	
...
function StartOpenVPNCLients(){
	...
	while (list ($num, $ligne) = each ($tbl) ){
		if(preg_match("#/etc/artica-postfix/openvpn/clients/([0-9]+)#",$num,$re)){
			echo "Starting......: ".date("H:i:s")." OpenVPN client ID {$re[1]}\n";
...
function BuildIpTablesClient($eth,$tun_id){
	...
	if($eth==null){
		echo "Starting......: ".date("H:i:s")." OpenVPN no prerouting set (IPTABLES_ETH)\n";
		return false;
	}
	
	echo "Starting......: ".date("H:i:s")." OpenVPN tun$tun_id Enable IP Forwarding\n";
    shell_exec2("sysctl -w net.ipv4.ip_forward=1");         
   
	shell_exec2("$iptables -A INPUT -i tun$tun_id -j ACCEPT -m comment --comment \"ArticaVPNClient_$tun_id\"");
	shell_exec2("$iptables -A FORWARD -i tun$tun_id -j ACCEPT -m comment --comment \"ArticaVPNClient_$tun_id\"");
	shell_exec2("$iptables -A OUTPUT -o tun$tun_id -j ACCEPT -m comment --comment \"ArticaVPNClient_$tun_id\"");
	shell_exec2("$iptables -t nat -A POSTROUTING -o $eth -j MASQUERADE -m comment --comment \"ArticaVPNClient_$tun_id\"");
	
	//iptables -t nat -A POSTROUTING -i tun8 -o eth0 -j MASQUERADE
	//iptables -t nat -A POSTROUTING -i eth0 -o tun0 -j MASQUERADE
	

	shell_exec2("$iptables -A INPUT -i $eth -j ACCEPT -m comment --comment \"ArticaVPNClient_$tun_id\"");
	shell_exec2("$iptables -A FORWARD -i $eth -j ACCEPT -m comment --comment \"ArticaVPNClient_$tun_id\"");
	shell_exec2("$iptables -A OUTPUT -o $eth -j ACCEPT -m comment --comment \"ArticaVPNClient_$tun_id\"");
	shell_exec2("$iptables -t nat -A POSTROUTING -o tun$tun_id -j MASQUERADE -m comment --comment \"ArticaVPNClient_$tun_id\"");
	
	echo "Starting......: ".date("H:i:s")." OpenVPN prerouting success for tun$tun_id...\n";
...
function OpenVPNCLientStart($id){
	...
	if(!is_numeric($id)){echo "Starting......: ".date("H:i:s")." OpenVPN client $id is not numeric (".__LINE__.")\n";return;}
	if($id==0){echo "Starting......: ".date("H:i:s")." OpenVPN client $id is not a valid integer (".__LINE__.")\n";return;}
	
	if(!is_file("$main_path/$id/settings.ovpn")){
		BuildOpenVpnSingleClient($id);
		if(!is_file("$main_path/$id/settings.ovpn")){
			echo "Starting......: ".date("H:i:s")." OpenVPN client $id, unable to stat $main_path/$id/settings.ovpn (".__LINE__.")\n";
			return; 
		} 
	}
	...
	if($unix->process_exists($pid)){
		echo "Starting......: ".date("H:i:s")." OpenVPN client $id, Already running PID $pid\n";
		return;
	}
	BuildOpenVpnSingleClient($id);
	...
	if(!$bridge){
		...
		if($tun<>null){
			...
			echo "Starting......: ".date("H:i:s")." OpenVPN client TUN $id,creating dev \"$tun\"\n";
		...
		echo "Starting......: ".date("H:i:s")." OpenVPN client TAP $id,creating dev \"$tap\"\n";
		system("$openvpn --mktun --dev $tap");
	}
	
	if(is_file("$main_path/$id/auth-user-pass")){
		echo "Starting......: ".date("H:i:s")." OpenVPN client [$id] authentication is enabled...\n";
		$EnableAuth=" --auth-user-pass $main_path/$id/auth-user-pass";
	}
	
	echo "Starting......: ".date("H:i:s")." OpenVPN client [$id] log file will be $main_path/$id/openvpn-status.log\n";
	...
	for($i=0;$i<7;$i++){
		$count++;
		echo "Starting......: ".date("H:i:s")." OpenVPN client [$id] (pid=$pid), waiting for pid $i/7\n";
		if($unix->process_exists($pid)){break;}
		if($count>5){echo "Starting......: ".date("H:i:s")." OpenVPN client $id, time-out\n";break;}
		$pid=vpn_client_pid($id);
	...
	$pid=vpn_client_pid($id);
	if(!$unix->process_exists($pid)){
		echo "Starting......: ".date("H:i:s")." OpenVPN client $id, failed \"$cmd\"\n";
		iptables_delete_client_rules($id);
		return;
	}
	
	echo "Starting......: ".date("H:i:s")." OpenVPN client $id, success running pid number $pid\n";
	if(!$bridge){
		$ethlink=trim(@file_get_contents("$main_path/$id/ethlink"));
		
		if(trim($ethlink)==null){
			$ethlink=OpenVpnClientGetDefaultethLink();
			echo "Starting......: ".date("H:i:s")." OpenVPN client $id, no ethlink...create a default one for $ethlink\n";
		...
		if($ethlink<>null){
			...
			echo "Starting......: ".date("H:i:s")." OpenVPN client $id, no ethlink...in $main_path/$id/ethlink\n";
		}
	}
...
if($bridge){
	...
	//print_r($routes);
	//print_r($localnets);
	if(count($routes)==0){echo "Starting......: ".date("H:i:s")." OpenVPN no routes to add\n";return;}
...
function BuildOpenVpnClients_changeConfig($mainpath,$ethid){
	echo "Starting......: ".date("H:i:s")." OpenVPN client $mainpath/settings.ovpn\n";
	...
	while (list ($num, $ligne) = each ($f) ){
		if(preg_match("#^dev\s+tun(.*)#",$ligne,$re)){
			echo "Starting......: ".date("H:i:s")." OpenVPN client tun dev=". trim($re[1])." change to tun$ethid\n";
			$f[$num]="dev tun$ethid";
		}
		
		if(preg_match("#^dev\s+tap(.*)#",$ligne,$re)){
			echo "Starting......: ".date("H:i:s")." OpenVPN client tap dev=". trim($re[1])." change to tap$ethid\n";
			$f[$num]="dev tap$ethid";
		}		
...
function wakeup_client_mode(){
	...
	while (list ($path, $id) = each ($tbl) ){
			if(!preg_match("#/etc/artica-postfix/openvpn/clients/([0-9]+)#",$path,$re)){if($GLOBALS["VERBOSE"]){echo "Starting......: ".date("H:i:s")." $path NO MATCH\n";}continue;}
			$id=$re[1];
			if($GLOBALS["VERBOSE"]){echo "Starting......: ".date("H:i:s")." OpenVPN wake up checking client ID:$id\n";}
			if(!is_file("$path/settings.ovpn")){if($GLOBALS["VERBOSE"]){echo "Starting......: ".date("H:i:s")." $path/settings.ovpn no such file\n";}continue;}
...
function wakeup_client_mode_getWakeup($path){
	...
	if($GLOBALS["VERBOSE"]){echo "Starting......: ".date("H:i:s")." $path no wakeup ip addr\n";}
}
