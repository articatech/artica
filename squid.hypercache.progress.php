...
function title(){
	...
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("{APP_SQUID}::{HYPERCACHE_STOREID}");
	return $title;
}
...
if($prc==100){
	...
	function Start$time(){
		...
		document.getElementById('title-$t').innerHTML='$title';
		$('#progress-$t').progressbar({ value: $prc });
		LoadAjaxRound('squid-hypercache-table','squid.hypercache.php');
		RTMMailHide();
...
function Launch(){
	$sock=new sockets();
	$cmd="squid.php?hypercache-progress=yes";
