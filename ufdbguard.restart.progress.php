...
function title(){
	...
	$title=$tpl->_ENGINE_parse_body("{RECONFIGURING_WEBFILTERING}");
...
function js(){
	...
	$t=time();
	$tpl=new templates();
	$compile_squid_ask=$tpl->javascript_parse_text("{compile_ufdb_restart}");
	if($_GET["ask"]=="yes"){
...
function Launch(){
	...
	$cmd="squid.php?force-restart-ufdbguard=yes$ApplyConfToo";
	if(isset($_GET["onlySquid"])){
		$ApplyConfToo="&ApplyConfToo=yes";
