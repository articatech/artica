...
function popup_js(){
	...
	$q=new mysql();
	$sql="SELECT pattern FROM spamasssin_escrap WHERE `ID`='{$_GET["ID"]}'";
...
function popup_add(){
	...
	<div class=explain style='font-size:18px'>{spamassassin_escrap_explain}</div>
...
<script>
	var xSave$t= function (obj) {
		...
		YahooWin3Hide();
		$('#SPAMASSASSIN_ESCRAP_TABLE').flexReload();
	}
...
function SaveItems(){
	...
	while (list ($num, $ligne) = each ($items2) ){
		...
		$q=new mysql();
		$q->QUERY_SQL("INSERT IGNORE INTO spamasssin_escrap (pattern,zdate) VALUE ('$ligne',NOW())","artica_backup");
...
function remove(){
	$q=new mysql();
	$q->QUERY_SQL("DELETE FROM spamasssin_escrap WHERE ID='{$_POST["remove"]}'","artica_backup");
...
function table(){
	...
	$title=$tpl->javascript_parse_text("{escrap_rules}");
...
$buttons="
	buttons : [
		...
		$html="
		<table class='SPAMASSASSIN_ESCRAP_TABLE' style='display: none' id='SPAMASSASSIN_ESCRAP_TABLE' style='width:100%'></table>
		<script>
		var xsite='';
		$(document).ready(function(){
		$('#SPAMASSASSIN_ESCRAP_TABLE').flexigrid({
...
var xRemoveSpamAssassineScrap= function (obj) {
	var tempvalue=obj.responseText;
...
function RemoveSpamAssassineScrap(ID){
	var XHR = new XHRConnection();
	XHR.appendData('remove',ID);
	XHR.sendAndLoad('$page', 'POST',xRemoveSpamAssassineScrap);
	}
