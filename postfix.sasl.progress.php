...
function js(){
	...
	$title=$tpl->javascript_parse_text("{authentication}::{$_GET["hostname"]}");
	$filename=$_GET["filename"];
	$filename_enc=urlencode($filename);
	echo "
	YahooWinBrowseHide();		
	RTMMail('800','$page?popup=yes&hostname={$_GET["hostname"]}','$title');";
...
function FilesLogs(){
	...
	$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/web/SMTP_SASL_PROGRESS";
	$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/web/SMTP_SASL_PROGRESS.txt";
...
function popup(){
	...
	$sock->getFrameWork("cmd.php?postfix-sasl-mech=yes&hostname={$_GET["hostname"]}");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{authentication}::{$_GET["hostname"]}...");
