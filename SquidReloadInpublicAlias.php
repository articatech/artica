<?php
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/SquidReloadInpublicAlias.progress";
...
function title(){
	$tpl=new templates();
	return $tpl->javascript_parse_text("{reloading_service}");
}
...
function js(){
	...
	$t=time();
	$html="
	<center id='$t' style='width:1024px' class=form></center>		
	<script>LoadAjax('$t','$page?popup=yes');</script>";
	$tpl =new template_users("{reloading_service}",$html,1);
	echo $tpl->web_page;
...
function Launch(){
	$sock=new sockets();
	$cmd="squid2.php?SquidReloadInpublicAlias=yes";
...
function popup(){
	...
	$title="{PLEASE_WAIT_RELOADING_SERVICES}";
...
$html="
<center id='title-$t' style='font-size:22px;margin-bottom:20px'>$text</center>
