...
function title(){
	...
	$title=$tpl->_ENGINE_parse_body("{new_server}::$ArticaMetaUsername@{$ArticaMetaHost}:$ArticaMetaPort");
	return $title;
}
...
if($prc==100){
	...
	function Start$time(){
		...
		document.getElementById('title-$t').innerHTML='$title';
		$('#progress-$t').progressbar({ value: $prc });
		$('#ARTICA_META_MAIN_TABLE_SWITCH').flexReload();
		setTimeout('RTMMailHide();',5000);
		}
	setTimeout(\"Start$time()\",1000);
	";	
	return;	
...
function Launch(){
	$sock=new sockets();
	$cmd="artica.php?meta-add-node=yes";
