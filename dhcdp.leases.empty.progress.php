...
function js(){
	...
	$title=$tpl->javascript_parse_text("{empty_leases}");
	$ask=$tpl->javascript_parse_text("{empty_leases_ask}");
	echo "
	if(confirm('$ask')){
		YahooWinBrowseHide();		
		RTMMail('800','$page?popup=yes&source-t={$_GET["source-t"]}','$title');
	}";
...
function Start$time(){
		...
		Loadjs('$page?build-js=yes&t=$t&md5file={$_GET["md5file"]}&source-t={$_GET["source-t"]}');
}
...
if($md5file<>$_GET["md5file"]){
	...
	var xStart$time= function (obj) {
		...
		Loadjs('$page?build-js=yes&t=$t&md5file=$md5file&source-t={$_GET["source-t"]}');
	}		
	...
	function Start$time(){
		...
		XHR.appendData('source-t','{$_GET["source-t"]}');
		XHR.appendData('t', '$t');
		XHR.setLockOff();
...
function Start$time(){
		...
		$('#progress-$t').progressbar({ value: $prc });
		RTMMailHide();
		if(document.getElementById('flexRT{$_GET["source-t"]}')){ 
			$('#flexRT{$_GET["source-t"]}').flexReload();
		}
...
function popup(){
	...
	$sock->getFrameWork("dhcpd.php?empty-leases-progress=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{empty_leases}...");
...
function Step1$t(){
	...
	Loadjs('$page?build-js=yes&t=$t&md5file=0&source-t={$_GET["source-t"]}');
}
$('#progress-$t').progressbar({ value: 1 });
