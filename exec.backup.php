...
function CleanContainers($ID,$mount_path_final){
	...
	while (list ($directory, $line) = each ($temparay) ){
	...
	if(count($ListedDirs)>$ExecBackupMaxContainers){
		...
		ksort($ListedDirs);
		$c=0;
		while (list ($time, $directory) = each ($ListedDirs) ){
...
function backup_mkdir($path){
	...
	if(is_file($mkdir)){
		...
		if(count($results)>0){while (list ($num, $line) = each ($results)){writelogs("MKDIR Found $line",__FUNCTION__,__FILE__,__LINE__);}}
		
		if(is_file($chmod)){
			exec("$chmod -R 0755 \"". dirname($path)."\" 2>&1",$results);
			if(count($results)>0){while (list ($num, $line) = each ($results)){writelogs("CHMOD Found $line",__FUNCTION__,__FILE__,__LINE__);}}
		}
