<?php
	...
	if(isset($_POST["ValvuladEnabled"])){ValvuladEnabled();exit;}
...
function tabs(){
	...
	echo build_artica_tabs($tab, "main_valvuad_daemon",1490)."<script>LeftDesign('management-console-256.png');</script>";
...
function popup_index(){
	...
	$EnablePolicydWeight_field=Paragraphe_switch_img('{APP_VALVUAD}',
			'{APP_VALVUAD_TEXT}','ValvuladEnabled',
			$ValvuladEnabled,"{enable_disable}",1450);
	
	

	
	
	$html="<div style='font-size:30px;margin-bottom:50px'>{APP_VALVUAD}</div>
...
function Save$t(){
	...
	XHR.appendData('ValvuladEnabled',document.getElementById('ValvuladEnabled').value);
...
function ValvuladEnabled(){
	$sock=new sockets();
	$sock->SET_INFO("ValvuladEnabled", $_POST["ValvuladEnabled"]);
