...
function js(){
	...
	$file=$_GET["file"];
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("{APP_SAMBA}::$file");
	@unlink(dirname(__FILE__)."/ressources/logs/web/samba.downgrade.html");
	$compile_squid_ask=$tpl->javascript_parse_text("{upgrade_samba_ask}");
	if($_GET["ask"]=="yes"){
...
function restart(){
	...
	$sock->getFrameWork("samba.php?downgrade=$fileenc");
...
function Filllogs(){
	$datas=explode("\n",@file_get_contents("ressources/logs/web/samba.downgrade.html"));
	krsort($datas);
	echo @implode("\n", $datas);
}
...
function logs(){
	...
	$t=$_GET["t"];
	$tt=time();
	$datas=@file_get_contents("ressources/logs/web/samba.downgrade.html");
	if(strlen($datas)<10){
