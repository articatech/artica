<?php
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/web/ufdbguard.compile.progress";
...
function title(){
	$tpl=new templates();
	return $tpl->javascript_parse_text("{APP_UFDBGUARD} {apply_restart}");
}
...
function js(){
	...
	$t=time();
	$tpl=new templates();
	$compile_squid_ask=$tpl->javascript_parse_text("{ufdb_apply_restart}");
...
function Launch(){
	$sock=new sockets();
	$cmd="ufdbguard.php?apply-restart-progress=yes";
