...
$GLOBALS["CACHEFILE"]="/usr/share/artica-postfix/ressources/logs/cyrus.mbx.progress";
$GLOBALS["LOGSFILES"]="/usr/share/artica-postfix/ressources/logs/web/cyrus.mbx.txt";
...
function js(){
	...
	$t=time();
	$title=$tpl->javascript_parse_text("{create_mailbox2}:{$_GET["uid"]}");
...
function suffix(){
	...
	return "&uid=$uidenc&MailBoxMaxSize=$MailBoxMaxSize";
}
...
function popup(){
	...
	$uidenc=urlencode($uid);
	$MailBoxMaxSize=$_GET["MailBoxMaxSize"];
	$sock->getFrameWork("cyrus.php?create-mbx=yes&uid=$uidenc&MailBoxMaxSize=$MailBoxMaxSize");
	$t=$_GET["t"];
	$suffix=suffix();
