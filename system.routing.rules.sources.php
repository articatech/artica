...
var xSave$t= function (obj) {
	...
	$('#TABLE_IP_RULE_{$_GET["ruleid"]}').flexReload();
}
function Save$t(){
...
function route_delete(){
	...
	$ligne=mysql_fetch_array($q->QUERY_SQL("DELETE FROM routing_rules_src WHERE ID='$zmd5'","artica_backup"));
...
function  route_delete_js(){
	...
	$q=new mysql();
	$database="artica_backup";
	$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT * FROM routing_rules_src WHERE ID='$ID'","artica_backup"));
...
function route_move(){
	...
	$q->QUERY_SQL("UPDATE routing_rules_src SET zOrder='$zOrder' WHERE zOrder='$NewzOrder' AND ID<>'$ID'",$database);
	$q->QUERY_SQL("UPDATE routing_rules_src SET zOrder='$NewzOrder' WHERE ID='$ID'",$database);
	
	$results=$q->QUERY_SQL("SELECT * FROM routing_rules_src WHERE ruleid={$_GET["ruleid"]} ORDER BY zOrder",$database);
	$c=0;
	while ($ligne = mysql_fetch_assoc($results)) {
		$c++;
		$q->QUERY_SQL("UPDATE routing_rules_src SET zOrder='$c' WHERE ID='{$ligne["ID"]}'",$database);
		if(!$q->ok){echo $q->mysql_error;}
	}
...
function route_js(){
	...
	if($ID>0){
		$q=new mysql();
		$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT `pattern` FROM routing_rules_src WHERE ID='$ID'","artica_backup"));
		$title=$ligne["pattern"];
...
function route_popup(){
	...
	$html="
		...
		<tr>
			<td class=legend style='font-size:24px'>{priority}:</td>
...
function NextRuleCheck$t(){
	...
	if(type==3){
		document.getElementById('gateway-$t').disabled=false;
	}
}
NextRuleCheck$t();
</script>
...
function route_save(){
	...
	if($ID==0){
		...
		$sql="INSERT INTO routing_rules_src (`type`,`gateway`,`pattern`,`ruleid`,`nic`,`metric`,`zOrder`)
		VALUES('{$_POST["type"]}','{$_POST["gateway"]}','{$_POST["pattern"]}','{$_POST["ruleid"]}','{$_POST["nic"]}','{$_POST["metric"]}','{$_POST["zOrder"]}');";
	}else{
		$sql="UPDATE routing_rules_src SET
				`metric`='{$_POST["metric"]}',
				`zOrder`='{$_POST["zOrder"]}',
	...
	if(!$q->FIELD_EXISTS("routing_rules_src", "metric", "artica_backup")){
		$q->QUERY_SQL("ALTER TABLE `routing_rules_src` ADD `metric` INT(10) NOT NULL, ADD INDEX (`metric`)","artica_backup");
	}
...
function table(){
	...
	// 	$sql="INSERT INTO routing_rules_src (`type`,`gateway`,`pattern`,`zmd5`,`nic`)
	...
	<table class='TABLE_IP_RULE_{$_GET["ruleid"]}' style='display: none' id='TABLE_IP_RULE_{$_GET["ruleid"]}' style='width:100%'></table>
	
	<script>
	var rowid=0;
	$(document).ready(function(){
	$('#TABLE_IP_RULE_{$_GET["ruleid"]}').flexigrid({
	...
	{display: '<span style=font-size:16px>$items</span>', name : 'pattern', width : 244, sortable : true, align: 'right'},
...
function search(){
	...
	$t=$_GET["t"];
	$search='%';
	$table="routing_rules_src";
	$page=1;
	$FORCE_FILTER="AND ruleid='{$_GET["ruleid"]}'";
	$total=0;
	
	if(!$q->FIELD_EXISTS("routing_rules_src", "zOrder", "artica_backup")){
		$q->QUERY_SQL("ALTER TABLE `routing_rules_src` ADD `zOrder` INT(10) NOT NULL, ADD INDEX (`zOrder`)","artica_backup");
