...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/suricata.install.progress";
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/suricata.install.progress.txt";
...
function js(){
	...
	if($_GET["key"]<>null){
		$sock=new sockets();
		$ArticaTechNetSquidRepo=unserialize(base64_decode($sock->GET_INFO("ArticaTechNetSuricataRepo")));
...
if($prc==100){
	...
	function Start$time(){
		...
		document.getElementById('title-$t').innerHTML='$title';
		$('#progress-$t').progressbar({ value: $prc });
		LoadAjaxRound('influx-update','influx.update.php')
		LayersTabsAllAfter();
		RTMMailHide();
		CacheOff();
		}
...
function popup(){
	...
	$sock->getFrameWork("suricata.php?install-tgz=".urlencode($_GET["filename"])."&key={$_GET["key"]}&OS={$_GET["OS"]}");
	$t=$_GET["t"];
