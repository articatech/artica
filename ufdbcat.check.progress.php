...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/ufdbcat.check.progress";
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/web/ufdbcat.check.progress.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{test_categories_rate}");
	
	echo "
	YahooWinBrowseHide();		
	RTMMail('800','$page?popup=yes&TARGETFILE=".urlencode($_GET["filename"])."','$title');
	";
...
function buildjs(){
	...
	$TARGETFILE=urlencode($_GET["TARGETFILE"]);
...
function Start$time(){
		...
		Loadjs('$page?build-js=yes&t=$t&md5file=$md5file&TARGETFILE=$TARGETFILE&filename=".urlencode($cachefile)."');
}
...
function Start$time(){
		...
		document.getElementById('title-$t').innerHTML='$title';
		$('#progress-$t').progressbar({ value: $prc });
		Loadjs('$page?build-js=yes&t=$t&md5file={$_GET["md5file"]}&TARGETFILE=$TARGETFILE&filename=".urlencode($_GET["filename"])."');
	}
...
function popup(){
	...
	$sock->getFrameWork("squid2.php?CategoriesRate=".urlencode($_GET["TARGETFILE"])."");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{test_categories_rate}...");
...
function Step1$t(){
	...
	Loadjs('$page?build-js=yes&t=$t&md5file=0&filename=".urlencode($_GET["filename"])."&TARGETFILE=".urlencode($_GET["TARGETFILE"])."');
}
$('#progress-$t').progressbar({ value: 1 });
