...
function stats_dahsboard_title(){
	...
	$tr[]="<a href=\"javascript:GoToWebsites$t();\" $style>{domains}</a>";
...
function build_query_js(){
	...
	if(intval($ligne["ID"])==0){
		$md5=md5("SMTP_UNIQ:$from$to$interval$USER_FIELD$USER_DATA");
		...
		('$md5','$title','SMTP_UNIQ',NOW(),'$serialize')";
		$q->QUERY_SQL($sql);
...
function build_websites(){
	...
	$zmd5=$_GET["zmd5"];
	$tt=time();
	$SITE=$tpl->javascript_parse_text("{recipients}");
	$zDate=$tpl->javascript_parse_text("{zDate}");
	$size=$tpl->javascript_parse_text("{size}");
	$hits=$tpl->javascript_parse_text("{messages}");
	$chronology=$tpl->javascript_parse_text("{domains}");
	...
	dataType: 'json',
	colModel : [
	{display: '<span style=font-size:18px>$SITE</span>', name : 'todomain', width : 514, sortable : true, align: 'left'},
...
function graph2(){
	...
	$q=new postgres_sql();
	$sql="SELECT SUM(size) as size,todomain FROM \"$table\" GROUP BY todomain ORDER BY size DESC LIMIT 10";
	$results=$q->QUERY_SQL($sql);
	...
	$highcharts->PiePlotTitle="{domains}";
	$highcharts->Title=$tpl->_ENGINE_parse_body("{recipient}/{size} (MB)");
	echo $highcharts->BuildChart();
...
function table1(){
	...
	$results=$q->QUERY_SQL("SELECT SUM(size) as size,todomain FROM \"$table\" GROUP BY todomain ORDER BY size DESC LIMIT 15");
	$html[]="<table style='width:100%'>";
	while($ligne=@pg_fetch_assoc($results)){
		$site=$ligne["todomain"];
		$size=FormatBytes($ligne["size"]/1024);
...
function build_chronology_table(){
	...
	$zmd5=$_GET["zmd5"];
	$tt=time();
	$SITE=$tpl->javascript_parse_text("{recipient}");
	$zDate=$tpl->javascript_parse_text("{zDate}");
	$size=$tpl->javascript_parse_text("{size}");
	$hits=$tpl->javascript_parse_text("{messages}");
	$chronology=$tpl->javascript_parse_text("{chronology}");
	$receipient=$tpl->javascript_parse_text("{recipient}");
	$html="
	...
	{display: '<span style=font-size:18px>$SITE</span>', name : 'tomail', width : 514, sortable : true, align: 'left'},
	{display: '<span style=font-size:18px>$hits</span>', name : 'rqs', width : 151, sortable : true, align: 'left'},
	{display: '<span style=font-size:18px>$size</span>', name : 'size', width : 151, sortable : true, align: 'left'},
	...
	$buttons
	searchitems : [
	{display: '$receipient', name : 'tomail'},
	],
	sortname: 'zDate',
	sortorder: 'asc',
	usepager: true,
...
function build_websites_search(){
	...
	$table="(SELECT SUM(size) as size, SUM(rqs) as rqs, todomain FROM \"$table\" GROUP by todomain ORDER by size desc) as t";
