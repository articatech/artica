...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/web/squid.statistics.not-categorized.progress";
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/web/squid.statistics.not-categorized.progress.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{not_categorized} {analyze}");
...
if($prc==100){
	...
	function Start$time(){
		...
		$('#progress-$t').progressbar({ value: $prc });
			RTMMailHide();
			LoadAjaxRound('not-categorized-div','squid.statistics.not-categorized.php');
...
function popup(){
	...
	$sock->getFrameWork("squid2.php?not-categorize-progress=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{not_categorized} {analyze}");
