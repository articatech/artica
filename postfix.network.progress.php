...
function js(){
	...
	$title=$tpl->javascript_parse_text("{recompile_postfix_parameters}");
	echo "RTMMail('800','$page?popup=yes$restart&hostname={$_GET["hostname"]}','$title')";
...
function buildjs(){
	...
	$time=time();
	$MEPOST=0;
	$cachefile="/usr/share/artica-postfix/ressources/logs/postfix.othervalues.progress";
	$logsFile="/usr/share/artica-postfix/ressources/logs/postfix.othervalues.progress.log";
	header("content-type: application/x-javascript");
	$tpl=new templates();
	$page=CurrentPageName();
...
function Start$time(){
		...
		Loadjs('$page?build-js=yes&t=$t&hostname={$_GET["hostname"]}$restart');
}
...
//Loadjs('$page?build-js=yes&t=$t&hostname={$_GET["hostname"]}');
...
function popup(){
	...
	$restart=null;
	if($_GET["restart"]=="yes"){$restart="&restart=yes"; }
	$sock->getFrameWork("postfix.php?apply-networks-master=yes&hostname={$_GET["hostname"]}");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{recompile_postfix_parameters}<br>{please_wait_preparing_settings}...");
