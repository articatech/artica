...
function build_query_js(){
	...
	$t=$_GET["t"];
	$user=$_GET["user"];
	$md5=md5("SMTP_MEMBERS:$from$to$interval$user$search");
	...
	if(intval($ligne["ID"])==0){
		...
		$title="{messaging} {members}: $timetext1 - $timetext2 - $user/$search";
		$sql="INSERT IGNORE INTO `reports_cache` (`zmd5`,`title`,`report_type`,`zDate`,`params`) VALUES
		('$md5','$title','SMTP_MEMBERS',NOW(),'$serialize')";
		$q->QUERY_SQL($sql);
	...
	function Start$t(){
		if(document.getElementById('POSTFIX_STATISTICS_MEMBERS_MD5')){
			document.getElementById('POSTFIX_STATISTICS_MEMBERS_MD5').md5='$md5';
		}
...
function table1(){
	...
	$html="
	<input type='hidden' id='POSTFIX_STATISTICS_MEMBERS_MD5' value='$zmd5'>
	<table class='POSTFIX_STATISTICS_MEMBERS' style='display: none' id='POSTFIX_STATISTICS_MEMBERS' style='width:99%'></table>
	<script>
	$(document).ready(function(){
	$('#POSTFIX_STATISTICS_MEMBERS').flexigrid({
