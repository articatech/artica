...
function js(){
	...
	echo "YahooWinBrowse('700','$page?manual-update=yes&ruleid={$_GET["ruleid"]}','$title',true)";
...
function PathUploaded(fileName){
	LoadAjax('file-uploader-demo1','$page?file-uploader-demo1=yes&fileName='+fileName+'&ruleid={$_GET["ruleid"]}');
}
createUploader$t();
</script>
";
...
function upload_artica_perform(){
	...
	$content_dir=dirname(__FILE__)."/img/";
...
function upload_artica_final(){
	...
	$text=$tpl->_ENGINE_parse_body("<div style='font-size:16px'>{saving} $fileName</div>");
	...
	<script>
	LoadAjaxTiny('$t','$page?uncompress=$fileNameEnc&ruleid={$_GET["ruleid"]}');
	</script>
