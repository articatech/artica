...
function build_progress_idb($text,$pourc){
	$GLOBALS["CACHEFILE"]="/usr/share/artica-postfix/ressources/logs/proftpd.install.progress";
	echo "{$pourc}% $text\n";
...
function install($filekey=0,$OS){
	...
	if($filekey<>0){
		$sock=new sockets();
		$ArticaTechNetHaProxyRepo=unserialize(base64_decode($sock->GET_INFO("ArticaTechNetProFTPDRepo")));
	...
	if(!is_file("/etc/init.d/proftpd")){
		system("$php /usr/share/artica-postfix/exec.initslapd.php --proftpd");
	}
	...
	if($GLOBALS["PROGRESS"]){
		...
		system("$php /usr/share/artica-postfix/exec.status.php --proftpd >/dev/null");
