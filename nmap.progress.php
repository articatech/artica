...
function js(){
	...
	echo "RTMMail('800','$page?popup=yes&MAC={$_GET["MAC"]}&ipaddr={$_GET["ipaddr"]}','$title')";
...
function buildjs(){
	...
	$logsFile="/usr/share/artica-postfix/ressources/logs/web/nmap_single_progress.txt";
	header("content-type: application/x-javascript");
	$tpl=new templates();
	$page=CurrentPageName();
...
function popup(){
	...
	$page=CurrentPageName();
	$sock=new sockets();
	$sock->getFrameWork("system.php?nmap-scan-single=yes&MAC={$_GET["MAC"]}&ipaddr={$_GET["ipaddr"]}");
	$t=$_GET["t"];
