...
function page(){
	...
	$title=$tpl->_ENGINE_parse_body("{APP_SS5}: {proxy_service_events}");
...
$sock=new sockets();
	...
	if($_POST["query"]<>null){
		...
		$sock->getFrameWork("ss5.php?ss5events=$search&rp={$_POST["rp"]}");
		
		$datas=unserialize(@file_get_contents("/usr/share/artica-postfix/ressources/logs/web/ss5-events"));
		$total=count($datas);
		
	}else{
		$sock->getFrameWork("ss5.php?ss5events=&rp={$_POST["rp"]}");
		$datas=unserialize(@file_get_contents("/usr/share/artica-postfix/ressources/logs/web/ss5-events"));
		$total=count($datas);
	}
	...
	while (list ($key, $line) = each ($datas) ){
		...
		if(strpos($line,'$S5LoadConfData$')>0){continue;}
		if(strpos("  $line",'Configuration not switched')>0){continue;}
		
		$line=str_replace('[ERRO]', "<span style='color:#680000'>", $line);
		if(preg_match("#(completed|starting)#i", $line)){$line="<span style='color:#003D0D;font-weight:bold'>$line</line>";}
		
		$c++;
		$data['rows'][] = array(
			'id' => md5($line),
			'cell' => array(
					"<span style='font-size:16px'>$datetext</span>", 
					"<span style='font-size:16px'>$line</span></span>")
		);
	}
