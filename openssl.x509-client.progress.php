...
function title(){
	$tpl=new templates();
	return $tpl->javascript_parse_text("{generate_x509_client}: {$_GET["generate-x509"]}");
}


function suffix(){
	$CommonName=$_GET["generate-x509"];
	$CommonName=urlencode($CommonName);
	return "&generate-x509=$CommonName";
}
...
function Launch(){
	$sock=new sockets();
	$cmd="openssl.php?generate-x509-client=".urlencode($_GET["generate-x509"]);
