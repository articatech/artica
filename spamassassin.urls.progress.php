...
$GLOBALS["LOGSFILES"]="/usr/share/artica-postfix/ressources/logs/web/spamassassin.urls.progress.log";
...
function title(){
	...
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("{antispam_rules}");
	return $title;
}
...
function Launch(){
	$sock=new sockets();
	$cmd="milter-spamass.php?urls-database=yes";
...
function popup(){
	...
	$text=$tpl->_ENGINE_parse_body("{please_wait} &raquo;").title();
