...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/postgres.progress";
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/postgres.progress.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{restarting} PostgreSQL");
	$t=time();
	$ask=null;
...
if($prc==100){
	...
	function Start$time(){
		...
		LayersTabsAllAfter();
		RTMMailHide();
		if(document.getElementById('influxdb_main_table')){
...
function popup(){
	...
	$text=$tpl->_ENGINE_parse_body("{please_wait_preparing_settings}...<br>{restarting} PostgreSQL");
