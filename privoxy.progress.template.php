...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/privoxy.template.progress";
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/privoxy.template.progress.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{APP_PRIVOXY} {error_page}");
...
if($prc==100){
	...
	function Start$time(){
		...
		$('#progress-$t').progressbar({ value: $prc });
			RTMMailHide();
			if( document.getElementById('main_privoxy_tabs') ){
				RefreshTab('main_privoxy_tabs');
			}
...
function popup(){
	...
	$sock->getFrameWork("privoxy.php?template=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{APP_PRIVOXY} {error_page}...");
