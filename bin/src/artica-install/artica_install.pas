...
zlogs.commandlog();
 ...
 if ParamStr(1)='--ejabberd' then  begin
   SYS.verbosed:=true;
   zejabberd:=tejabberd.Create(SYS);
   writeln('Version: ',zejabberd.VERSION());
   halt(0);
end;
...
if paramStr(1)='--artica-boot' then begin
    install:=Tclass_install.Create;
    install.install_artica_boot();
end;
...
if ParamStr(1)='--lighttpd-perms' then begin
   halt(0);
end;
if ParamStr(1)='--lighttpd-phpmyadmin' then begin
   zlighttpd.PHPMYADMIN();
   halt(0);
end;
...
if ParamStr(1)='--samba-reload' then begin
   samba3:=tsamba.Create;
   samba3.RELOAD();
   halt(0);
end;
...
if ParamStr(1)='--nsswitch' then begin
   ...
   if ParamStr(2)='--start' then fpsystem('/etc/init.d/slapd start');
   halt(0);
end;
...
if ParamStr(1)='--slapdconf' then begin
   zldap:=tOpenldap.Create;
...
if ParamStr(1)='--zpush-version' then begin
   ztoolsversions:=ttoolsversions.Create(SYS);
   writeln(ztoolsversions.ZPUSH_VERSION());
   exit;
end;
...
if ParamStr(1)='--squid-rebuild-caches' then begin
   if not SYS.BuildPids() then begin
       writeln('Already instance running');
       halt(0);
   end;
   zsquid:=Tsquid.Create;
   zsquid.rebuild_caches();
   halt(0);
end;
...
if ParamStr(1)='--reconfigure-cyrus' then begin
   ...
   if DisableIMAPVerif=0  then begin
...
if  ParamStr(1)='-geoip-updates' then halt(0);
