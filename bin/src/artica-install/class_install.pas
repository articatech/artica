...
uses
  ...
  mailgraph_daemon, miltergreylist,lighttpd, roundcube,dansguardian,kav4samba,stunnel4,dkimfilter,kav4proxy,bind9,obm,mysql_daemon,p3scan,syslogng,openvpn,cups,
  ...
  emailrelay,mldonkey,backuppc,kav4fs,ocsi,ocsagent,auditd,squidguard_page,dkfilter,squidguard,framework,dkimmilter,dropbox,articapolicy,virtualbox,tftpd,crossroads,articaexecutor,pptpd,
  ...
  mailarchiver in '/home/dtouzeau/developpement/artica-postfix/bin/src/artica-install/mailarchiver.pas',ddclient,tomcat,openemm,fuppes,memcached,updateutilityhttp,ejabberd,arkeia,klms,suricata,
...
begin
    ...
    if FileExists('/etc/nologin') then begin
       writeln('***** System stopping, aborting task '+ParamStr(2)+' *****');
       halt(0);
    end;
   if not SYS.BuildPids() then begin
       logs.Debuglogs('Already instance running');
       halt(0);
   end;
           ...
           if ParamStr(2)='UpdateUtility' then begin
               UpdateUtility:=tUpdateUtility.Create(GLOBAL_INI.SYS);
               UpdateUtility.STOP();
               UpdateUtility.free;
               exit();
            end;
           ...
           if ParamStr(2)='ejabberd' then begin
               ejabberd:=tejabberd.Create(GLOBAL_INI.SYS);
               ejabberd.STOP();
               ejabberd.free;
               exit();
            end;

           if ParamStr(2)='haproxy' then begin

               exit();
            end;

           if ParamStr(2)='arkeia' then begin
               arkeia:=arkeia.Create(GLOBAL_INI.SYS);
               arkeia.STOP();
               arkeia.free;
               exit();
            end;
           ...
           if ParamStr(2)='articadb' then begin
               articadb:=tarticadb.Create(GLOBAL_INI.SYS);
               articadb.STOP();
               articadb.free;
               exit();
            end;
          ...
          if ParamStr(2)='mimedefang' then begin
            ...
            if ParamStr(2)='arpd' then begin
               fpsystem('/etc/init.d/arpd stop');
               exit();
            end;

            if ParamStr(2)='netatalk' then begin
               netatalk:=tnetatalk.Create(GLOBAL_INI.SYS);
               netatalk.STOP();
               netatalk.free;
               exit();
            end;

            if ParamStr(2)='memcached' then begin
               memcached:=memcached.Create(GLOBAL_INI.SYS);
               memcached.STOP();
               memcached.free;
               exit();
            end;
           ...
           if ParamStr(2)='fcron' then begin
              ...
              if ParamStr(2)='zarafa-dagent' then begin
                 zarafa:=tzarafa_server.Create(SYS);
                 zarafa.DAGENT_STOP();
                 exit();
              end;
              ...
              if ParamStr(2)='yaffas' then begin
                 zyaffas:=tyaffast.Create(SYS);
                 zyaffas.STOP();
                 exit();
              end;

              if ParamStr(2)='loadbalance' then begin
                 fpsystem(SYS.LOCATE_PHP5_BIN()+' /usr/share/artica-postfix/exec.loadbalance.php --stop');
                 exit();
              end;
              ...
              if ParamStr(2)='zarafa-server' then begin
               ...
               if ParamStr(2)='zarafa-search' then begin
                 zarafa:=tzarafa_server.Create(SYS);
                 zarafa.SEARCH_STOP();
                 exit();
              end;

              if ParamStr(2)='zarafa-gateway' then begin
                 zarafa:=tzarafa_server.Create(SYS);
                 zarafa.GATEWAY_STOP();
                 exit();
              end;
             ...
             if ParamStr(2)='ufdb' then begin
                   writeln('Use init.d instead...');
                   fpsystem('/etc/init.d/ufdb stop');
                   exit();
              end;

             if ParamStr(2)='ufdb-tail' then begin
                   writeln('Use init.d instead...');
                   fpsystem('/etc/init.d/ufdb-tail stop');
                   exit();
              end;
            ...
            if ParamStr(2)='auditd' then begin
             ...
             if ParamStr(2)='squidguard-http' then begin
              ...
              if ParamStr(2)='fetchmail' then begin
                 fpsystem('/etc/init.d/fetchmail stop');
                 exit();
              end;
           ...
           if ParamStr(2)='kav4proxy' then begin
            ...
            if ParamStr(2)='roundcube' then begin
                 fpsystem(SYS.LOCATE_PHP5_BIN()+' /usr/share/artica-postfix/exec.roundcube.php --stop');
                 exit();
              end;
          ...
          if ParamStr(2)='p3scan' then begin
            ...
            if ParamStr(2)='samba' then begin
                 logs.Syslogs('Warning: Request to STOP Samba....');
                 zsamba.SAMBA_STOP();
                 cups:=tcups.CReate();
                 cups.STOP();
                 exit();
           ...
           if ParamStr(2)='winbindd' then begin
            ...
            if ParamStr(2)='daemon' then begin
                 logs.Syslogs('Request to stop all Artica managed services ');
                 GLOBAL_INI.ARTICA_STOP();
                 exit();
            end;
...
begin
   ...
   if not SYS.BuildPids() then begin
       writeln('Already instance running');
       halt(0);
   end;
    ...
    if not NetWorkAvailable then begin
       writeln('Warning, Network is unreachable, some services could not be start...');
             ...
             if ParamStr(2)='ldap' then begin
              ...
              if ParamStr(2)='loadbalance' then begin
                 fpsystem(SYS.LOCATE_PHP5_BIN()+' /usr/share/artica-postfix/exec.loadbalance.php --start');
                 exit();
              end;
              ...
              if ParamStr(2)='ufdb' then begin
                  fpsystem('/etc/init.d/ufdb start');
                  writeln('Use init.d instead');
                   exit();
              end;

              if ParamStr(2)='ufdb-tail' then begin
                   fpsystem('/etc/init.d/ufdb-tail start');
                   writeln('Use init.d instead');
                   exit();
              end;
              ...
              if ParamStr(2)='pptpd' then begin
               ...
               if ParamStr(2)='cluebringer' then begin
                ...
                if ParamStr(2)='UpdateUtility' then begin
                   SYS:=Tsystem.Create;
                   UpdateUtility:=tUpdateUtility.Create(SYS);
                   UpdateUtility.START();
                   exit();
              end;
           ...
           if ParamStr(2)='ejabberd' then begin
                 zejabberd:=tejabberd.Create(GLOBAL_INI.SYS);
                 zejabberd.START();
                 zejabberd.free;
                 exit();
            end;
           ...
           if ParamStr(2)='arkeia' then begin
                 arkeia:=tarkeia.Create(GLOBAL_INI.SYS);
                 arkeia.START();
                 arkeia.free;
                 exit();
            end;
          ...
          if ParamStr(2)='zarafadb' then begin
           ...
           if ParamStr(2)='klmsdb' then begin
            ...
            if ParamStr(2)='openemm-sendmail' then begin
             ...
             if ParamStr(2)='arpd' then begin
               fpsystem('/etc/init.d/arpd start');
               exit();
            end;
           ...
           if ParamStr(2)='amavis' then begin
            ...
            if ParamStr(2)='dansguardian-tail' then begin
             ...
             if ParamStr(2)='pptpd-clients' then begin
              ...
              if ParamStr(2)='samba' then begin
                 if not NetWorkAvailable then exit();
                 zlogs.Syslogs('INFO: Request to START Samba....');
                 zsamba.SAMBA_START();
                 cups:=tcups.CReate();
                 cups.START();
                 exit();
             ...
             if ParamStr(2)='winbindd' then begin
                 zlogs.Syslogs('INFO: Request to START Winbindd....');
                 zsamba.SAMBA_WINBINDD_START();
                 exit();
            end;

             if ParamStr(2)='daemons' then begin
                zlogs.Syslogs('INFO: Request to START All services managed by Artica....');
...
begin
  ...
  fpsystem('/etc/init.d/cyrus-imapd restart');
  ...
  if FileExists(SYS.LOCATE_GENERIC_BIN('zarafa-server')) then fpsystem('/usr/share/artica-postfix/bin/artica-install --zarafa-ssl --rebuild');
...
//##############################################################################
procedure Tclass_install.install_artica_boot();
var
   myFile : TStringList;
   GLOBALINI:myconf;
   D:boolean;
   LOG:Tlogs;
begin
 if FileExists('/etc/init.d/artica-boot') then exit;
 GLOBALINI:=myconf.create;
 LOG:=Tlogs.create;
D:=GLOBALINI.COMMANDLINE_PARAMETERS('debug');
myFile:=TstringList.Create;
myFile.Add('#!/bin/sh');
myFile.add('### BEGIN INIT INFO');
myFile.Add('# Provides:          artica-boot');
myFile.Add('# Required-Start:    $local_fs');
myFile.Add('# Required-Stop:     $local_fs');
myFile.Add('# Should-Start:');
myFile.Add('# Should-Stop:');
myFile.Add('# Default-Start:     2 3 4 5');
myFile.Add('# Default-Stop:      0 1 6');
myFile.Add('# Short-Description: Start Artica boot start scripts');
myFile.Add('# chkconfig: 2345 11 89');
myFile.Add('# description: artica-boot script');
myFile.add('### END INIT INFO');
myFile.Add('case "$1" in');
myFile.Add(' start)');
myFile.Add('    ' +SYS.LOCATE_PHP5_BIN()+' /usr/share/artica-postfix/exec.checkfolder-permissions.php');
myFile.Add('    ;;');
myFile.Add('');
myFile.Add('  stop)');
myFile.Add('    ' +SYS.LOCATE_PHP5_BIN()+' /usr/share/artica-postfix/exec.checkfolder-permissions.php');
...
if DirectoryExists('/usr/local/etc/rc.d') then begin
   myfile.SaveToFile('/usr/local/etc/rc.d/artica-boot');
   fpsystem('/bin/ln -s /usr/local/etc/rc.d/artica-boot /etc/init.d/artica-boot');
end else begin
   myfile.SaveToFile('/etc/init.d/artica-boot');
end;

fpsystem('/bin/chmod +x /etc/init.d/artica-boot >/dev/null 2>&1');

 if FileExists('/usr/sbin/update-rc.d') then begin
    fpsystem('/usr/sbin/update-rc.d -f artica-boot defaults >/dev/null 2>&1');
 end;

  if FileExists('/sbin/chkconfig') then begin
     fpsystem('/sbin/chkconfig --add artica-boot >/dev/null 2>&1');
     fpsystem('/sbin/chkconfig --level 2345 artica-boot on >/dev/null 2>&1');
  end;
  myFile.free;
