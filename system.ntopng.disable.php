...
function title(){
	...
	$tpl=new templates();
	$sock=new sockets();
	return $tpl->javascript_parse_text("Disable NtopNG");
...
function js(){
	...
	$t=time();
	$tpl=new templates();
	$disable_ntopng_ask=$tpl->javascript_parse_text("{disable_ntopng_ask}");
	
		$warn="if(!confirm('$disable_ntopng_ask')){return;}";
...
function Launch(){
	$sock=new sockets();
	$cmd="system.php?disable-ntopng=yes";
