...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/influxdb-restart.progress";
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/web/influxdb.restart.progress.txt";
...
function js(){
	...
	$server=$_GET["server"];
	$title=$tpl->javascript_parse_text("{restore}:$server");
	
	echo "
	YahooWinBrowseHide();		
	RTMMail('800','$page?popup=yes&server=".urlencode($_GET["server"])."','$title');
	";
...
function Start$time(){
		...
		Loadjs('$page?build-js=yes&t=$t&md5file=$md5file&filename=".urlencode($cachefile)."&server=$server');
}
...
if($md5file<>$_GET["md5file"]){
	...
	function Start$time(){
		...
		XHR.appendData('server','".urlencode($server)."');
		XHR.appendData('t', '$t');
		XHR.setLockOff();
...
function popup(){
	...
	$restart=null;
	$server=$_GET["server"];
	$sock->getFrameWork("influx.php?restore-progress-server=yes&server=$server");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{restore}...");
...
function Step1$t(){
	...
	Loadjs('$page?build-js=yes&t=$t&md5file=0&filename=".urlencode($_GET["filename"])."&server=$server');
}
$('#progress-$t').progressbar({ value: 1 });
