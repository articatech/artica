...
function js(){
	...
	$title=$tpl->javascript_parse_text("{IDS}:{reloading}");
...
if($prc==100){
	...
	function Start$time(){
		...
		$('#progress-$t').progressbar({ value: $prc });
			RTMMailHide();
			if( document.getElementById('suricata-tabs') ){
				RefreshTab('suricata-tabs');
			}
...
function popup(){
	...
	$sock->getFrameWork("suricata.php?reload-progress=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{IDS} {reloading}...");
