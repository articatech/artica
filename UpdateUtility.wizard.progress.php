...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/UpdateUtility.wizard.progress";
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/web/UpdateUtility.wizard.progress.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{APP_KASPERSKY_UPDATE_UTILITY} {configuration_wizard}");
...
if($prc==100){
	...
	function Start$time(){
		...
		LoadAjaxRound('kaspersky-update-utility','UpdateUtility.php');
...
function popup(){
	...
	$sock->getFrameWork("updateutility.php?wizard=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{APP_KASPERSKY_UPDATE_UTILITY} {configuration_wizard}");
