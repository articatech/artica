...
function popup(){
	...
	$title=$tpl->javascript_parse_text("{whitelisted}");
...
$('#table-$t').flexigrid({
	...
	colModel : [
		...
		{display: '<span style=font-size:18px>$hosts</span>', name : 'ipsrc', width : 622, sortable : true, align: 'left'},
...
function proxies_delete(){
	$sql="DELETE FROM windowsupdates_white WHERE ID={$_POST["proxy-delete"]}";
	$q=new mysql_squid_builder();
	$q->QUERY_SQL($sql);
	if(!$q->ok){echo $q->mysql_error;return;}
...
//ini_set('html_errors',0);ini_set('display_errors', 1);ini_set('error_reporting', E_ALL);ini_set('error_prepend_string','');ini_set('error_append_string','');
	...
	$sql="CREATE TABLE IF NOT EXISTS `windowsupdates_white` (
