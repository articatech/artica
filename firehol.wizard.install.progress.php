...
function title(){
	...
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("{install_firewall}");
	return $title;
}
...
function js(){
	...
	if(isset($_GET["ask"])){
		$ask="if(!confirm('".$tpl->javascript_parse_text("{reinstall_firewall}")."')){return;}";
...
if($prc==100){
	...
	function Start$time(){
		...
		LoadAjaxRound('dashboard-firewall','dashboard.firewall.php');
		}
	setTimeout(\"Start$time()\",1000);
	";	
	return;	
...
function Launch(){
	...
	$cmd="firehol.php?install-progress=yes";
...
function popup(){
	...
	$page=CurrentPageName();
	Launch();
	$text=$tpl->_ENGINE_parse_body("{install_firewall}");
	$t=time();
