...
function title(){
	...
	$tpl=new templates();
	$sock=new sockets();
	return $tpl->_ENGINE_parse_body("{build_webfiltering_rules}");
}
...
if($prc==100){
	...
	function Start$time(){
		...
		if(document.getElementById('main-ufdb-frontend')){
...
function Launch(){
	$sock=new sockets();
	$cmd="squid.php?ufdbguard-compile-smooth2=yes";
...
function popup(){
	...
	$title="{webfiltering} {compile_rules}";
