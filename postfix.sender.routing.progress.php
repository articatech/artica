...
function js(){
	...
	$title=$tpl->javascript_parse_text("{routing_table}::{$_GET["hostname"]}");
...
function popup(){
	...
	$sock->getFrameWork("postfix.php?apply_sender_routing_rule=yes&hostname={$_GET["hostname"]}");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{routing_table}::{$_GET["hostname"]}...");
