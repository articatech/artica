...
function page(){
	...
	if($_GET["zmd5"]==null){
		$q=new mysql_squid_builder();
		$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT title,zmd5 FROM reports_cache WHERE report_type='IDS' ORDER BY zDate DESC LIMIT 0,1"));
	}else{
		$q=new mysql_squid_builder();
...
function graph2(){
	...
	$results=$q->QUERY_SQL("SELECT SUM(xcount) as xcount,src_ip FROM \"$table\" GROUP BY src_ip ORDER BY xcount DESC LIMIT 10");
	while($ligne=@pg_fetch_assoc($results)){
		$size=$ligne["xcount"];
		$src_ip=$ligne["src_ip"];
		$PieData[$src_ip]=$size;
