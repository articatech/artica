...
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/web/clamav.freshclam.progress.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{APP_FRESHCLAM}");
...
if($prc==100){
	...
	function Start$time(){
		...
		LoadAjaxRound('clamav-updates-section','clamav.updates.php');
...
function popup(){
	...
	$sock->getFrameWork("clamav.php?freshclam-services=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{APP_FRESHCLAM}...");
