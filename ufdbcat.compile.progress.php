<?php
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/web/ufdbcat.restart.progress";
$GLOBALS["CACHEFILE"]=$GLOBALS["PROGRESS_FILE"];
$GLOBALS["LOGSFILES"]="/usr/share/artica-postfix/ressources/logs/web/ufdbcat.log";
...
function title(){
	$tpl=new templates();
	return $tpl->javascript_parse_text("{APP_UFDBCAT} {restarting_service}");
}
...
if($prc==100){
	...
	function Start$time(){
		...
		$('#progress-$t').progressbar({ value: $prc });
		RTMMailHide();
		RefreshTab('main_ufdbcat_config');
	}
	setTimeout(\"Start$time()\",1000);
	";	
	return;	
...
function Launch(){
	$sock=new sockets();
	$cmd="squid.php?ufdbcat-restart=yes";
...
function popup(){
	...
	$title="{PLEASE_WAIT_RESTARTING_SERVICE}";
