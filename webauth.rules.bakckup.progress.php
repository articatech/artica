...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/webauth.rules.bakckup.progress";
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/webauth.rules.bakckup.progress.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{backup} {rules}");
	$t=time();
	$ask=null;
	if($_GET["key"]<>null){
		$sock=new sockets();
		$ArticaTechNetSquidRepo=unserialize(base64_decode($sock->GET_INFO("ArticaTechNetHaProxyRepo")));
...
if($prc==100){
	...
	function Start$time(){
		...
		document.getElementById('title-$t').innerHTML='$title';
		$('#progress-$t').progressbar({ value: $prc });
		//hotspot.rules.backup.gz
		Loadjs('webauth.rules.php?backup-download-js=yes');
		RTMMailHide();
...
function popup(){
	...
	$sock->getFrameWork("hotspot.php?backup-rules=yes");
	$t=$_GET["t"];
