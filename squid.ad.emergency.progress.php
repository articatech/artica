...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/web/squid.urgency.disable.progress";
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/web/squid.ad.emergency.progress.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{emergency} Active Directory");
	$ask=$tpl->javascript_parse_text("{activedirectory_emergency_mode_explain2} ?");
	echo "
	if(confirm('$ask')){
		YahooWinBrowseHide();		
...
function popup(){
	...
	$sock->getFrameWork("squid2.php?emergency-activedirectory-progress=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{emergency} Active Directory");
