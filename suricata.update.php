...
function page(){
	$sock=new sockets();
	$ArticaTechNetInfluxRepo=unserialize(base64_decode($sock->GET_INFO("ArticaTechNetSuricataRepo")));
	$tpl=new templates();
	$version=$sock->GET_INFO("SuricataVersion");
	...
	while (list ($key, $array) = each ($ArticaTechNetInfluxRepo) ){
		...
		$button=button("{update2}","Loadjs('suricata.update.progress.php?key=$key&OS=$OS&filename=$FILENAME');",32);
