...
function js(){
	...
	$title=$tpl->javascript_parse_text("{build_languages}");
...
function FilesLogs(){
	...
	$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/web/locales.log";	
}
...
if($prc==100){
	...
	function Start$time(){
		...
		$('#progress-$t').progressbar({ value: $prc });
		RTMMailHide();
		if( document.getElementById('MAILBOX_ZARAFA_LANG_LIST') ){
				LoadAjax('MAILBOX_ZARAFA_LANG_LIST','create-user.php?MAILBOX_ZARAFA_LANG_LIST=yes');
			}
...
function popup(){
	...
	$filename=$_GET["filename"];
	$filename_enc=urlencode($filename);
	$sock->getFrameWork("services.php?locales-gen-progress=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{build_languages}...");
