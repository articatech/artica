...
function ShowID_js(){
	...
	$sql="SELECT subject FROM system_admin_mysql WHERE ID=$id";
	$q=new mysql();
...
$tpl=new templates();
$sql="SELECT content FROM system_admin_mysql WHERE ID={$_GET["ShowID"]}";
$q=new mysql();
$ligne=@mysql_fetch_array($q->QUERY_SQL($sql,"artica_events"));
...
function empty_table(){
	$q=new mysql();
	$q->QUERY_SQL("TRUNCATE TABLE system_admin_mysql","artica_events");
}
...
function events_table(){
	...
	if(!$q->TABLE_EXISTS($table, "artica_events")){json_error_show("no data ( no such table )",2);}
	if($q->COUNT_ROWS($table,"artica_events")==0){json_error_show("no data (now row)",2);}
