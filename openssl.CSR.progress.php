...
function title(){
	$tpl=new templates();
	return $tpl->javascript_parse_text("{certificate} {$_GET["generate-csr"]}");
}


function suffix(){
	return "&generate-csr=".urlencode($_GET["generate-csr"]);
}
...
function Launch(){
	$sock=new sockets();
	$cmd="openssl.php?generate-csr=".urlencode($_GET["generate-csr"]);
