...
function js(){
	...
	$title=$tpl->javascript_parse_text("{snapshots} {upload}");
	echo "
	YahooWinBrowseHide();		
...
function popup(){
	...
	$sock->getFrameWork("artica.php?snapshot-import=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{please_wait_importing_snapshot}...");
