...
function title(){
	...
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("{APP_SQUID}::{change_database_path}");
	if($_GET["backup"]=="yes"){
		$title=$tpl->_ENGINE_parse_body("{APP_SQUID}::{backup_database}");
	}
...
echo "//{$GLOBALS["LOGSFILES"]} / {$GLOBALS["CACHEFILE"]}\n";
echo "//PRC:$prc\n";	
...
function Launch(){
	$sock=new sockets();
	$cmd="squid.php?squid-db-change-database=yes";
	
	if($_GET["backup"]=="yes"){
		$cmd="squid.php?squid-db-backup-database=yes";
	}
...
function popup(){
	...
	if(isset($_GET["backup"])){
		$title="{please_wait}, {backup_database}";
