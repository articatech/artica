...
function compressToMonth(){
	...
	@file_put_contents("/etc/artica-postfix/settings/Daemons/HaproxyStatsMonthQueue", 0);
	
	if(system_is_overloaded()){
		@file_put_contents("/etc/artica-postfix/settings/Daemons/HaproxyStatsMonthQueue", 1);
	...
	while (list ($Day, $ligne) = each ($SOURCES_DAY) ){
		...
		$c++;
		if(!compress_day($Day)){@file_put_contents("/etc/artica-postfix/settings/Daemons/HaproxyStatsMonthQueue", 1);return;}
		if(system_is_overloaded()){
			$OverLoadedCount++;
...
function compressToYear($aspid=false){
	...
	@file_put_contents("/etc/artica-postfix/settings/Daemons/HaproxyStatsYearQueue", 0);
	
	if(system_is_overloaded()){
		@file_put_contents("/etc/artica-postfix/settings/Daemons/HaproxyStatsYearQueue", 1);
		squid_admin_mysql(0, "Overloaded system {$GLOBALS["SYSTEM_INTERNAL_LOAD"]}, aborting task", null,__FILE__,__LINE__);
		return;
	}
	...
	while (list ($Day, $ligne) = each ($SOURCES_MONTH) ){
		...
		$c++;
		if(!compress_month_perform($Day)){@file_put_contents("/etc/artica-postfix/settings/Daemons/HaproxyStatsYearQueue", 1);return;}
		if(system_is_overloaded()){
			$OverLoadedCount++;
			if($OverLoadedCount>$OverLoadedMax){
			squid_admin_mysql(0, "Overloaded system {$GLOBALS["SYSTEM_INTERNAL_LOAD"]}, aborting tasks after compressing $c days", null,__FILE__,__LINE__);
			@file_put_contents("/etc/artica-postfix/settings/Daemons/HaproxyStatsYearQueue", 1);return;}
		}
