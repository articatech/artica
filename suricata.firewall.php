...
function table(){
	...
	<table class='TABLE_SURICATA_MAIN_FIREWALL' style='display: none' id='TABLE_SURICATA_MAIN_FIREWALL'
	style='width:100%'></table>
	<script>
	$(document).ready(function(){
	var md5H='';
	$('#TABLE_SURICATA_MAIN_FIREWALL').flexigrid({
	url: '$page?list=yes',
	dataType: 'json',
	colModel : [
	...
	{display: '<span style=font-size:22px>$explain</span>', name : 'src_ip', width : 833, sortable : true, align: 'left'},
...
var xSuricataFwDelete= function (obj) {
	...
	$('#TABLE_SURICATA_MAIN_FIREWALL').flexReload();
}
...
function rules_list(){
	...
	if(isset($_POST["sortname"])){
		...
		if($searchstring<>null){
			...
			$sql="SELECT COUNT(*) AS tcount FROM suricata_firewall WHERE $searchstring";
		...
		$sql="SELECT * FROM suricata_firewall WHERE $searchstring $ORDER $limitSql";
		...
		while ($ligne = pg_fetch_assoc($results)) {
			...
			$data['rows'][] = array(
					'id' => $ligne['ID'],
					'cell' => array(
							"<strong style='font-size:18px;color:$color'>$signature_js{$ligne["signature"]}</a></strong>",
							"<strong style='font-size:18px;color:$color'>{$ligne["zdate"]}</strong>",
							"<span style='font-size:18px;color:$color'>$explain</span><br><i style='font-size:16px'>{$DESCZ[$ligne["signature"]]}</i>",
