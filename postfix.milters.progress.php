...
function js(){
	...
	$title=$tpl->javascript_parse_text("{filtering_modules}");
...
function popup(){
	...
	$sock=new sockets();
	$restart=null;
	$sock->getFrameWork("postfix.php?milters-progress=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{PLEASE_WAIT_UPDATING_MODULES}...");
