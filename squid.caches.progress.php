<?php
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/web/squid.caches.progress";
...
function title(){
	$tpl=new templates();
	return $tpl->javascript_parse_text("{verify_caches}");
}
...
function js(){
	...
	$warn=$tpl->javascript_parse_text("{check_caches_warning2}");
...
function Launch(){
	$sock=new sockets();
	$cmd="squid.php?verify-caches-progress=yes".suffix();
